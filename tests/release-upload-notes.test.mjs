...
const metadataPath = path.resolve(process.cwd(), '.skills', 'ospec-release-notes', 'releases', '8.8.8.json');

describe('release upload notes helpers', () => {
  it('resolves the local release metadata path under the release skill directory', () => {
    expect(releaseUploadNotes.getReleaseMetadataPath('8.8.8')).toBe(metadataPath);
  });

  it('parses the GitHub repository owner and name from package metadata', () => {
    expect(releaseUploadNotes.parseGitHubRepository('https://github.com/clawplays/ospec')).toEqual({
      owner: 'clawplays',
      repo: 'ospec',
    });
  });
