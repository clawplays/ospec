...
async function pathExists(targetPath) {
  try {
    await fsp.access(targetPath, nodeFs.constants.F_OK);
    return true;
  } catch {
    return false;
  }
...
async function readJson(filePath) {
  return JSON.parse(
    (await fsp.readFile(filePath, 'utf8')).replace(/^\uFEFF/, ''),
  );
}
...
function assertNotContains(output, unexpected, label) {
  if (output.includes(unexpected)) {
    throw new Error(
      `Expected ${label} to exclude "${unexpected}"\nActual output:\n${output}`,
    );
  }
}
...
async function main() {
  const tempDir = await fs.mkdtemp(
    path.join(os.tmpdir(), 'ospec-release-smoke-'),
  );
  ...
  try {
    ...
    assertContains(
      output,
      'Installed ospec Codex skill: ospec-change',
    ...
    output = run('node', [
      ...
      'install',
      'ospec-init',
      path.join(tempDir, 'codex-ospec-init'),
    ]);

    assertContains(
      output,
      'Installed ospec Codex skill: ospec-init',
      'explicit skill install output',
    );

    assertContains(
      await fs.readFile(
        path.join(tempDir, 'codex-ospec-init', 'skill.yaml'),
        'utf8',
      ),
    ...
    assertContains(
      output,
      'Installed ospec Claude Code skill: ospec-change',
      'claude skill install output',
    );
    ...
    output = run('node', [
      ...
      'install-claude',
      'ospec-init',
      path.join(tempDir, 'claude-ospec-init'),
    ]);

    assertContains(
      output,
      'Installed ospec Claude Code skill: ospec-init',
      'explicit claude skill install output',
    );

    if (
      !(await fs.pathExists(
        path.join(tempDir, 'claude-ospec-init', 'SKILL.md'),
      ))
    ) {
    ...
    assertContains(
      output,
      'Project initialized to change-ready state',
      'init output',
    );
    assertContains(
      output,
      'Protocol shell: created',
      'init protocol shell output',
    );
    assertContains(
      output,
      `Next: ospec new <change-name> "${projectDir}"`,
    ...
    assertContains(
      output,
      'Queued change queued-smoke created',
      'queue add output',
    );
    ...
    assertContains(
      output,
      `ospec queue add second-active "${projectDir}"`,
      'quoted queue suggestion',
    );
    ...
    assertNotContains(
      tasksContent,
      '- [ ] 1.',
      'tasks hybrid checklist numbering',
    );
    ...
    await fs.writeFile(
      tasksPath,
      (await fs.readFile(tasksPath, 'utf8')).replace(/- \[ \]/g, '- [x]'),
      'utf8',
    );
    ...
    assertContains(
      ...
      'archived proposal link',
    );

    assertContains(
      await fs.readFile(path.join(archivedFeatureDir, 'tasks.md'), 'utf8'),
      archivedOverviewLink,
      'archived tasks link',
    );
    ...
    assertContains(
      await fs.readFile(
        path.join(archivedFeatureDir, 'verification.md'),
        'utf8',
      ),
      archivedOverviewLink,
    ...
    assertContains(
      output,
      'Activated next queued change: queued-smoke',
      'queue next output',
    );
