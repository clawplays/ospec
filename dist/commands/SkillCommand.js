...
const PACKAGED_GLOBAL_SKILLS = ['ospec-change', 'ospec-goal'];
const ACTION_SKILLS = [
    {
        name: 'ospec-init',
        title: 'OSpec Init',
...
class SkillCommand extends BaseCommand_1.BaseCommand {
    ...
    getAvailableSkillNames() {
        return ['ospec', ...ACTION_SKILLS.map(skill => skill.name), ...PACKAGED_GLOBAL_SKILLS, 'ospec-cli'];
    }
    ...
    async getSkillDefinition(skillName) {
        ...
        if (PACKAGED_GLOBAL_SKILLS.includes(skillName)) {
            return this.buildPackagedGlobalSkillDefinition(skillName);
        }
    ...
    async buildPackagedGlobalSkillDefinition(skillName) {
        const packageRoot = this.resolvePackageRoot();
        const sourceRoot = path_1.default.join(packageRoot, 'assets', 'global-skills', 'codex', skillName);
        const sourceSkillMd = await services_1.services.fileService.readFile(path_1.default.join(sourceRoot, 'SKILL.md'));
        const sourceSkillYaml = await services_1.services.fileService.readFile(path_1.default.join(sourceRoot, 'skill.yaml'));
        const sourceOpenaiYaml = await services_1.services.fileService.readFile(path_1.default.join(sourceRoot, 'agents', 'openai.yaml'));
        const metadata = this.readSkillMarkdownMetadata(sourceSkillMd, skillName);
        return {
            name: skillName,
    ...
    readSkillMarkdownMetadata(markdown, fallbackName) {
        const description = markdown.match(/^description:\s*(.+)$/m)?.[1]?.trim() || `Managed ${fallbackName} skill.`;
        const title = markdown.match(/^#\s+(.+)$/m)?.[1]?.trim() || fallbackName;
        return {
            title,
            description,
            shortDescription: description.length > 80 ? description.slice(0, 77) + '...' : description,
        };
    }
