...
- Use \`ospec execute route [changes/active/<change>]\` to write \`artifacts/agents/workflow-route.json\` and \`artifacts/agents/workflow-route.md\` with the next recommended OSpec command; this command records workflow routing artifacts only and does not edit source files.
...
- Use \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]\` after dispatch to write the native agent launch plan; it tells the controlling AI how to use the current harness mechanism: Codex/GPT \`spawn_agent\`/\`wait_agent\`/\`close_agent\`, Claude Code Task, Gemini \`@generalist\`, OpenCode \`@mention\`, Cursor Agent/task chat, and Copilot CLI/coding-agent task. Use \`--json\` when an adapter needs the machine-readable launch artifact on stdout. It requires one active dispatch and ready workspace status, and does not start workers, run shell commands, or edit source files by itself.
...
- Use \`ospec execute debug [changes/active/<change>] --phase reproduce|isolate|hypothesize|fix|verify --symptom "..." --root-cause "..." --status FIXED\` when debugging is part of the change to record \`artifacts/agents/debug-evidence.json\`. This command records evidence only and does not run shell commands.

- Use \`ospec execute tdd [changes/active/<change>] --phase red|green|refactor --command "..."\` after focused test runs to record \`artifacts/agents/tdd-evidence.json\`. This command records evidence only and does not run shell commands; red must not pass, green requires a prior red FAILED record, refactor requires prior passing green/refactor evidence, and SKIPPED requires a concrete summary.
...
`,
    },
    {
        ...
        defaultPrompt: 'Use $ospec-archive to archive a completed OSpec change. Check readiness first, then run ospec archive on the active change path. If you only need a dry check, use ospec archive --check; do not stop after a passing dry check when full closeout was requested.',
...
- do not stop after a passing \`--check\` when the requested outcome is full closeout
...
class SkillCommand extends BaseCommand_1.BaseCommand {
    async execute(action = 'status', skillNameOrTargetDir, targetDir) {
        try {
            ...
            if (verb === 'author') {
                this.printAuthoringGuide(skillNameOrTargetDir);
                return;
            }
            if (verb === 'check') {
                const result = await this.checkAuthoredSkill(skillNameOrTargetDir);
                this.printSkillCheck(result);
                return;
            }
    ...
    resolveAction(action) {
        switch (action) {
            ...
            case 'author':
                return { provider: 'codex', verb: 'author' };
            case 'check':
                return { provider: 'codex', verb: 'check' };
            default:
    ...
    printAuthoringGuide(targetDir) {
        const resolvedTarget = path_1.default.resolve(targetDir || process.cwd());
        console.log('\nOSpec Skill Authoring');
        console.log('=====================\n');
        console.log(`Target: ${resolvedTarget}`);
        console.log('');
        console.log('Authoring workflow:');
        console.log('  1. Create SKILL.md with frontmatter name and description.');
        console.log('  2. Define trigger conditions, read order, commands, and guardrails.');
        console.log('  3. Add skill.yaml and agents/openai.yaml when packaging for Codex.');
        console.log('  4. Run ospec skill check <skill-dir> before installing or publishing.');
        console.log('  5. Run ospec skill install <skill-name> <target-dir> only for managed OSpec skills.');
        console.log('');
        console.log('Expected files:');
        console.log('  - SKILL.md');
        console.log('  - skill.yaml (Codex package only)');
        console.log('  - agents/openai.yaml (Codex package only)');
        console.log('');
        console.log('Next command:');
        console.log(`  ${(0, helpers_1.formatCliCommand)('ospec', 'skill', 'check', resolvedTarget)}`);
        console.log('');
    }
    async checkAuthoredSkill(targetDir) {
        const resolvedTarget = path_1.default.resolve(targetDir || process.cwd());
        const skillMdPath = path_1.default.join(resolvedTarget, 'SKILL.md');
        const skillYamlPath = path_1.default.join(resolvedTarget, 'skill.yaml');
        const openAiYamlPath = path_1.default.join(resolvedTarget, 'agents', 'openai.yaml');
        const checks = [];
        const add = (status, label, detail) => {
            checks.push({ status, label, detail });
        };
        const skillMdExists = await services_1.services.fileService.exists(skillMdPath);
        if (!skillMdExists) {
            add('FAIL', 'SKILL.md', 'Missing SKILL.md.');
        }
        else {
            const skillMd = await services_1.services.fileService.readFile(skillMdPath);
            add('PASS', 'SKILL.md', 'Found SKILL.md.');
            add(/^---\r?\n[\s\S]*?\r?\n---/.test(skillMd) ? 'PASS' : 'FAIL', 'frontmatter', 'SKILL.md should include YAML frontmatter.');
            add(/(^|\n)name:\s*\S+/.test(skillMd) ? 'PASS' : 'FAIL', 'name', 'Frontmatter should include name.');
            add(/(^|\n)description:\s*\S+/.test(skillMd) ? 'PASS' : 'FAIL', 'description', 'Frontmatter should include description.');
            add(/##\s+Commands|##\s+Workflow|##\s+Guardrails/i.test(skillMd) ? 'PASS' : 'WARN', 'workflow guidance', 'Skill should document workflow, commands, or guardrails.');
            const blockedTerms = [`super${'powers'}`, `ospec${'-src'}`];
            const blocked = blockedTerms.filter(term => skillMd.toLowerCase().includes(term));
            add(blocked.length === 0 ? 'PASS' : 'FAIL', 'publication labels', blocked.length === 0
                ? 'No blocked external or local source labels found.'
                : 'Blocked external or local source labels were found.');
        }
        if (await services_1.services.fileService.exists(skillYamlPath)) {
            const skillYaml = await services_1.services.fileService.readFile(skillYamlPath);
            add('PASS', 'skill.yaml', 'Found Codex skill metadata.');
            add(/default_prompt:\s*/.test(skillYaml) ? 'PASS' : 'WARN', 'default prompt', 'skill.yaml should include interface.default_prompt.');
        }
        else {
            add('WARN', 'skill.yaml', 'Missing skill.yaml; acceptable for non-Codex packages only.');
        }
        if (await services_1.services.fileService.exists(openAiYamlPath)) {
            add('PASS', 'agents/openai.yaml', 'Found OpenAI agent metadata.');
        }
        else {
            add('WARN', 'agents/openai.yaml', 'Missing agents/openai.yaml; acceptable for non-Codex packages only.');
        }
        return {
