...
5. \`Announce-Before-Act\`：绝不静默执行——宣告当前 skill 与阶段、即将运行的 \`ospec execute ...\` 命令与产物、所选 runtime adapter、worker 数量及实际机制（已验证 Orca terminal、harness-native、target CLI 或串行 current controller），以及被哪个门禁阻塞。
...
9. dispatch 后用 \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]\` 写入 \`launch-plan.md\`；machine-readable launch artifact 的 \`runtimeAdapter\` 按已验证 Orca 工作区、当前有效 harness-native capability、可用目标 CLI、串行 generic 当前控制器排序。Orca 进程名本身不是归属证据，必须由 CLI status 和 current-worktree 探测共同确认。
9. 默认多 worker 执行服从 \`runtimeAdapter.selected\`；所选 adapter 支持并行时启动整个并行安全 batch，首选探测失败时继续安全降级。普通任务可退回当前控制器串行执行，独立 review 没有独立 adapter 时仍必须阻断。
9. 只有 \`runtimeAdapter.selected\` 或记录的 fallback order 选择显式 CLI orchestration 路径时，才用 \`ospec execute orchestrate [changes/active/<change>] --command "..." [--limit N] [--max-rounds N] [--timeout-ms N]\`；只并发运行 adapter 允许的 batch，记录 \`artifacts/agents/orchestration-runs/\`，并把结果 collect 回 task graph。
9. 所选 target-CLI adapter 需要单 worker runner，或更高优先级 adapter 在 claim ownership 前失败时，用 \`--run --command\`（即 \`ospec execute launch ... --run --command "..."\`）并写入 \`artifacts/agents/worker-runs/\`；随后用 \`ospec execute collect [changes/active/<change>] [--task task-id] [--run run-id]\` 记录结果。修复 blocked、needs-context 或 failed work 后，用 \`ospec execute retry\` 重新派发；已完成任务必须显式 \`--force\` 才能 retry。
...
5. \`Announce-Before-Act\`: never run the workflow silently — announce which OSpec skill and stage you are in, which \`ospec execute ...\` command you will run and the artifact it writes, the selected runtime adapter, how many workers it will start, the actual mechanism (verified Orca terminal, Claude Code \`Task\`, Codex/GPT \`spawn_agent\`, Gemini \`@generalist\`, OpenCode \`@mention\`, target CLI, or serial current controller), and which gate is blocking when progress stops.
...
9. After dispatch, use \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]\` to write \`launch-plan.md\`. The machine-readable launch artifact's \`runtimeAdapter\` orders verified Orca workspace ownership, current harness-native capability, an available target CLI, and the serial generic current controller. An Orca process name alone is never ownership evidence; both CLI status and current-worktree probes must succeed.
9. Default multi-worker execution follows \`runtimeAdapter.selected\`: launch the whole parallel-safe batch when that adapter supports parallel work and continue through safe fallbacks when a preferred probe fails. Ordinary tasks may fall back to serial current-controller work; independent review remains blocked without an independent adapter.
9. Use \`ospec execute orchestrate [changes/active/<change>] --command "..." [--limit N] [--max-rounds N] [--timeout-ms N]\` only when \`runtimeAdapter.selected\` or its recorded fallback order selects an explicit CLI orchestration path; it renders the command template, runs only the adapter-supported parallel batch, records \`artifacts/agents/orchestration-runs/\`, collects results into the task graph, and reports failed-worker retry commands.
9. Use \`--run --command\` (that is, \`ospec execute launch ... --run --command "..."\`) when the selected target-CLI adapter needs the single-worker runner or an earlier adapter fails before claiming ownership; it writes \`artifacts/agents/worker-runs/\`. Then use \`ospec execute collect [changes/active/<change>] [--task task-id] [--run run-id]\` to record the result. Use \`ospec execute retry\` to reopen corrected blocked, needs-context, or failed work; completed tasks require \`--force\`.
...
5. \`Announce-Before-Act\`: ワークフローを黙って実行しない。どの skill・段階か、これから実行する \`ospec execute ...\` コマンドと生成物、選択された runtime adapter、起動する worker 数、実際の機構（検証済み Orca terminal、Claude Code \`Task\`、Codex/GPT \`spawn_agent\`、Gemini \`@generalist\`、OpenCode \`@mention\`、target CLI、または serial current controller）、どのゲートでブロックしているかを伝える。
...
9. dispatch 後は \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]\` で \`artifacts/agents/launch-plan.md\` を書く。machine-readable artifact の \`runtimeAdapter\` は、検証済み Orca workspace、current harness-native capability、利用可能な target CLI、serial generic current controller の順で解決する。Orca の process 名だけでは ownership の証拠にならない。
9. multi-worker execution は \`runtimeAdapter.selected\` に従う。選択 adapter が parallel execution をサポートするときだけ parallel-safe batch を並列起動し、優先 adapter の probe が失敗した場合は記録された安全な fallback order を続行する。通常 task は serial current controller に fallback できるが、independent review には independent adapter が必要である。
9. \`ospec execute orchestrate [changes/active/<change>] --command "..." [--limit N] [--max-rounds N] [--timeout-ms N]\` は、\`runtimeAdapter.selected\` または記録された fallback order が explicit CLI orchestration path を選択した場合だけ使う。adapter が許可した batch だけを実行し、\`artifacts/agents/orchestration-runs/\` を記録して task graph に collect する。
9. 選択された target-CLI adapter が single-worker runner を必要とする場合、または前の adapter が ownership claim 前に失敗した場合は、\`--run --command\`（つまり \`ospec execute launch ... --run --command "..."\`）を使い、\`artifacts/agents/worker-runs/\` を記録する。その後 \`ospec execute collect [changes/active/<change>] [--task task-id] [--run run-id]\` で result を記録する。blocked/needs-context/failed work の修正後は \`ospec execute retry\` で再 dispatch する。完了済み task は \`--force\` が必要。
...
5. \`Announce-Before-Act\`: لا تُشغّل سير العمل بصمت — أعلن أي skill ومرحلة، وأي أمر \`ospec execute ...\` ستشغّله وما يكتبه، وruntime adapter المختار، وعدد workers، والآلية الفعلية (Orca terminal موثّق، أو Claude Code \`Task\`، أو Codex/GPT \`spawn_agent\`، أو Gemini \`@generalist\`، أو OpenCode \`@mention\`، أو target CLI، أو serial current controller)، وأي بوابة تحجب التقدم.
...
9. بعد dispatch، استخدم \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]\` لكتابة \`artifacts/agents/launch-plan.md\`؛ يحل \`runtimeAdapter\` بالترتيب: Orca workspace موثّق، ثم current harness-native capability، ثم target CLI متاح، ثم serial generic current controller. لا يكفي اسم process خاص بـ Orca لإثبات ownership.
9. يتبع multi-worker execution القيمة \`runtimeAdapter.selected\`: لا يشغّل parallel-safe batch بالتوازي إلا عندما يدعم adapter المختار ذلك، ويتابع fallback order الآمن المسجل عند فشل preferred probe. يمكن للـ task العادي الرجوع إلى serial current controller، أما independent review فيتطلب independent adapter.
9. استخدم \`ospec execute orchestrate [changes/active/<change>] --command "..." [--limit N] [--max-rounds N] [--timeout-ms N]\` فقط عندما يختار \`runtimeAdapter.selected\` أو fallback order المسجل مسار explicit CLI orchestration؛ شغّل فقط batch الذي يسمح به adapter، وسجل \`artifacts/agents/orchestration-runs/\`، ثم اجمع النتائج إلى task graph.
9. استخدم \`--run --command\` (أي \`ospec execute launch ... --run --command "..."\`) عندما يحتاج target-CLI adapter المختار إلى single-worker runner أو يفشل adapter سابق قبل claim ownership؛ يسجل ذلك \`artifacts/agents/worker-runs/\`. بعدها استخدم \`ospec execute collect [changes/active/<change>] [--task task-id] [--run run-id]\` لتسجيل النتيجة. بعد إصلاح blocked أو needs-context أو failed work استخدم \`ospec execute retry\` لإعادة dispatch؛ المهام المكتملة تحتاج \`--force\`.
...
\`Announce-Before-Act\`：绝不静默执行——宣告当前 skill 与阶段、即将运行的 \`ospec execute ...\` 命令与产物、所选 runtime adapter、worker 数量及实际机制（已验证 Orca terminal、harness-native、target CLI 或串行 current controller），以及被哪个门禁阻塞。
...
dispatch 后用 \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]\` 写入 \`launch-plan.md\`；machine-readable launch artifact 的 \`runtimeAdapter\` 按已验证 Orca 工作区、当前有效 harness-native capability、可用目标 CLI、串行 generic 当前控制器排序，Orca 进程名本身不是归属证据。
默认多 worker 执行服从 \`runtimeAdapter.selected\`；所选 adapter 支持并行时启动整个安全 batch，首选探测失败时自动降级。普通任务可串行回退，独立 review 没有独立 adapter 时必须阻断。
...
只有 \`runtimeAdapter.selected\` 或记录的 fallback order 选择显式 CLI orchestration 路径时，才用 \`ospec execute orchestrate [changes/active/<change>] --command "..." [--limit N] [--max-rounds N] [--timeout-ms N]\`，并且只并发运行 adapter 允许的 batch。
所选 target-CLI adapter 需要单 worker runner，或更高优先级 adapter 在 claim ownership 前失败时，用 \`--run --command\`（即 \`ospec execute launch ... --run --command "..."\`）并写入 \`artifacts/agents/worker-runs/\`；随后用 \`ospec execute collect [changes/active/<change>] [--task task-id] [--run run-id]\` 记录结果。修复 blocked、needs-context 或 failed work 后，用 \`ospec execute retry\` 重新派发；已完成任务必须显式 \`--force\` 才能 retry。
...
\`ospec session\`、\`ospec execute bootstrap\`、\`handoff\`、\`doc-review\`、\`workspace\`、plan 模式 \`worktree\`、\`finish\`、\`dispatch\`、\`launch\`、\`collect\`、\`retry\`、\`complete\`、\`review\`、\`feedback\`、\`debug\`、\`tdd\`、\`verify\` 与 \`sync\` 只更新 OSpec artifacts；除 \`workspace\`、\`worktree\` 与 \`finish\` 会读取 git 状态外，不直接编辑项目 source files。controller 按所选 runtime adapter 启动 worker。
...
After dispatch, use \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]\` to write \`launch-plan.md\`. The machine-readable launch artifact's \`runtimeAdapter\` orders verified Orca ownership, current harness-native capability, an available target CLI, and the serial generic current controller; an Orca process name alone is not ownership evidence.
Default multi-worker execution follows \`runtimeAdapter.selected\`: launch the whole safe batch when parallel execution is supported and fall back automatically when a preferred probe fails. Ordinary work may run serially; independent review blocks without an independent adapter.
...
Use \`ospec execute orchestrate [changes/active/<change>] --command "..." [--limit N] [--max-rounds N] [--timeout-ms N]\` only when \`runtimeAdapter.selected\` or its recorded fallback order selects an explicit CLI orchestration path; run only the adapter-supported batch, record \`artifacts/agents/orchestration-runs/\`, collect results into the task graph, and report failed-worker retry commands.
Use \`--run --command\` (that is, \`ospec execute launch ... --run --command "..."\`) when the selected target-CLI adapter needs the single-worker runner or an earlier adapter fails before claiming ownership; it writes \`artifacts/agents/worker-runs/\`. Then use \`ospec execute collect [changes/active/<change>] [--task task-id] [--run run-id]\` to record the result. Use \`ospec execute retry\` to reopen corrected blocked, needs-context, or failed work; completed tasks require \`--force\`.
...
\`Announce-Before-Act\`: ワークフローを黙って実行しない。skill・段階、コマンドと生成物、選択された runtime adapter、worker 数、実際の機構、blocking gate を伝える。
...
dispatch 後は \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]\` で \`artifacts/agents/launch-plan.md\` を書く。\`runtimeAdapter\` は検証済み Orca workspace、current harness-native capability、利用可能な target CLI、serial generic current controller の順で解決する。Orca の process 名だけでは ownership の証拠にならない。
multi-worker execution は \`runtimeAdapter.selected\` に従う。選択 adapter が parallel work をサポートするときだけ parallel-safe packet を並列起動する。通常 task は serial current controller に fallback できるが、independent review には independent adapter が必要である。
...
\`ospec execute orchestrate [changes/active/<change>] --command "..." [--limit N] [--max-rounds N] [--timeout-ms N]\` は、\`runtimeAdapter.selected\` または記録された fallback order が explicit CLI orchestration path を選択した場合だけ使い、adapter が許可した batch だけを実行する。
選択された target-CLI adapter が single-worker runner を必要とする場合、または前の adapter が ownership claim 前に失敗した場合は、\`--run --command\`（つまり \`ospec execute launch ... --run --command "..."\`）を使い、\`artifacts/agents/worker-runs/\` を記録する。その後 \`ospec execute collect [changes/active/<change>] [--task task-id] [--run run-id]\` で result を記録する。blocked/needs-context/failed work の修正後は \`ospec execute retry\` で再 dispatch する。完了済み task は \`--force\` が必要。
...
\`Announce-Before-Act\`: لا تُشغّل سير العمل بصمت — أعلن أي skill ومرحلة، وأي أمر \`ospec execute ...\` ستشغّله وما يكتبه، وruntime adapter المختار، وعدد workers، والآلية الفعلية (Orca terminal موثّق، أو harness-native، أو target CLI، أو serial current controller)، وأي بوابة تحجب التقدم.
...
يتبع multi-worker execution القيمة \`runtimeAdapter.selected\`: لا يشغّل parallel-safe batch بالتوازي إلا عندما يدعم adapter المختار ذلك. يمكن للـ task العادي الرجوع إلى serial current controller، أما independent review فيتطلب independent adapter.
...
استخدم \`ospec execute orchestrate [changes/active/<change>] --command "..." [--limit N] [--max-rounds N] [--timeout-ms N]\` فقط عندما يختار \`runtimeAdapter.selected\` أو fallback order المسجل مسار explicit CLI orchestration، وشغّل فقط batch الذي يسمح به adapter.
استخدم \`--run --command\` (أي \`ospec execute launch ... --run --command "..."\`) عندما يحتاج target-CLI adapter المختار إلى single-worker runner أو يفشل adapter سابق قبل claim ownership، وسجل \`artifacts/agents/worker-runs/\`. بعدها استخدم \`ospec execute collect [changes/active/<change>] [--task task-id] [--run run-id]\` لتسجيل النتيجة. بعد إصلاح blocked أو needs-context أو failed work استخدم \`ospec execute retry\` لإعادة dispatch؛ المهام المكتملة تحتاج \`--force\`.
