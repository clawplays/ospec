...
9. dispatch 后用 \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot] [--dry-run] [--json]\` 写入 \`launch-plan.md\`；\`runtimeAdapter\` 只接受当前、target 匹配的模型原生 subagent capability，并给出 native primitive。
...
9. After dispatch, use \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot] [--dry-run] [--json]\` to write \`launch-plan.md\`. Its \`runtimeAdapter\` accepts only a current, target-bound model-native subagent capability and exposes the native primitive.
...
9. dispatch 後は \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot] [--dry-run] [--json]\` で launch plan を書く。\`runtimeAdapter\` は current かつ target-bound な model-native subagent capability のみを受け入れる。
...
9. بعد dispatch، استخدم \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot] [--dry-run] [--json]\` لكتابة launch plan؛ يقبل \`runtimeAdapter\` فقط model-native subagent capability حالية ومرتبطة بالـ target.
...
dispatch 后用 \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot] [--dry-run] [--json]\` 写入 \`launch-plan.md\`；只有与 target 匹配、仍在有效期内的当前 session capability 才能选择 \`runtimeAdapter.selected.nativeSubagent\`。
...
After dispatch, use \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot] [--dry-run] [--json]\` to write \`launch-plan.md\`. Only a current session capability that matches the target may select \`runtimeAdapter.selected.nativeSubagent\`.
...
dispatch 後は \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot] [--dry-run] [--json]\` で \`artifacts/agents/launch-plan.md\` を書く。target と一致し、有効期限内の current session capability だけが \`runtimeAdapter.selected.nativeSubagent\` を選択できる。
...
بعد dispatch، استخدم \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot] [--dry-run] [--json]\` لكتابة \`artifacts/agents/launch-plan.md\`؛ لا يختار \`runtimeAdapter.selected.nativeSubagent\` إلا current session capability مطابقة للـ target وغير منتهية.
