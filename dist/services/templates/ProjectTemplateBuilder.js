...
6. \`implementation-plan.md\` 必须从 \`design.md\` 推导，并明确目标文件、预期结果、验证命令、依赖、可并行任务和冲突；\`artifacts/agents/task-graph.json\` 必须从 \`implementation-plan.md\` 推导；\`tasks.md\` 必须从 task graph 推导。生成的 \`parallelizable: false\` task 必须包含 \`serial_reason\`，显式限制单 worker 时记录 \`maxParallelReason\`；超过六个 \`target_files\` 的 task 必须拆分，或填写具体的 \`scope_reason\`；L3 白名单优先从任务图 derive/check/apply，重复 configure 是替换而非追加。
...
9. 多 worker 执行服从 \`runtimeAdapter.selected.nativeSubagent\`；只并行派发安全 batch。capability 缺失、target 不匹配或过期时必须阻断，不得启动 agent CLI 或退回当前 controller。所有 native wait 单次最多 60 秒；这只是 controller poll 上限，不是 child 运行上限。每个 live child 在 \`heartbeatDueAt\` 前续租，完成后用 action 的 \`loop finalize\` 原子提交证据和结果，并在每轮 poll 后重新 tick。capacity 未知时 implementation 批次最多两个，但不降低安全 review 并行度。
...
6. Derive \`implementation-plan.md\` from \`design.md\`, including target files, expected results, verification commands, dependencies, parallelizable work, and conflicts; derive \`artifacts/agents/task-graph.json\` from \`implementation-plan.md\`; derive \`tasks.md\` from the task graph. Generated \`parallelizable: false\` tasks require \`serial_reason\`, and an explicit single-worker limit requires \`maxParallelReason\`; split tasks with more than six \`target_files\` or add a concrete \`scope_reason\`; for L3 derive/check/apply exact task-graph permissions because repeated configure calls replace rather than append.
...
6. \`implementation-plan.md\` は \`design.md\` から導き、対象ファイル、期待結果、検証コマンド、依存関係、並行可能な作業、競合を記録する。\`artifacts/agents/task-graph.json\` は \`implementation-plan.md\` から導き、\`tasks.md\` は task graph から導く。生成した \`parallelizable: false\` task には \`serial_reason\`、明示的な single-worker limit には \`maxParallelReason\` が必要。6 個を超える \`target_files\` を持つ task は分割するか具体的な \`scope_reason\` を記録する。L3 は task graph から exact permissions を derive/check/apply し、configure の反復を append とみなさない。
...
6. يجب اشتقاق \`implementation-plan.md\` من \`design.md\` مع الملفات المستهدفة والنتائج المتوقعة وأوامر التحقق والاعتماديات والعمل القابل للتوازي والتعارضات؛ ويجب اشتقاق \`artifacts/agents/task-graph.json\` من \`implementation-plan.md\`؛ ويجب اشتقاق \`tasks.md\` من task graph. تتطلب المهام المولدة ذات \`parallelizable: false\` قيمة \`serial_reason\`، ويتطلب حد worker واحد الصريح \`maxParallelReason\`؛ قسّم task التي تتجاوز ستة \`target_files\` أو أضف \`scope_reason\` واضحاً؛ وفي L3 استخدم derive/check/apply ولا تفترض أن configure يضيف القيم.
...
\`implementation-plan.md\` 必须从 \`design.md\` 推导，并明确目标文件、预期结果、验证命令、依赖、可并行任务和冲突；\`artifacts/agents/task-graph.json\` 必须从 \`implementation-plan.md\` 推导；\`tasks.md\` 必须从 task graph 推导。串行 task 写 \`serial_reason\`，单 worker 限制写 \`maxParallelReason\`；超过六个 \`target_files\` 的 task 必须拆分或填写具体的 \`scope_reason\`；L3 从任务图 derive/check/apply 白名单。
...
多 worker 执行服从 \`runtimeAdapter.selected.nativeSubagent\`；只在所选 native adapter 支持并行时启动整个安全 batch。capability 缺失、过期或 target 不匹配时必须阻断，不能回退到 Orca、agent CLI 或 current controller。所有 native wait 单次最多 60 秒；这只是 controller poll 上限，不是 child 运行上限。每个 live child 在 \`heartbeatDueAt\` 前续租，完成后用 action 的 \`loop finalize\` 原子提交证据和结果，并在每轮 poll 后重新 tick。capacity 未知时 implementation 批次最多两个，但不降低安全 review 并行度。
...
When the change must pause for a user choice, record a durable gate with \`ospec execute decision [changes/active/<change>] --id <id> --question "..." --option id:label:impact --option id:label:impact [--recommended id] [--required]\`, present the decision report \`Chat Prompt\` or \`artifacts/agents/decisions/index.md\`, then record the selected option with \`ospec execute decision [changes/active/<change>] --id <id> --select <option-id> --answered-by user\`.
Derive \`implementation-plan.md\` from \`design.md\`, including target files, expected results, verification commands, dependencies, parallelizable work, and conflicts; derive \`artifacts/agents/task-graph.json\` from \`implementation-plan.md\`; derive \`tasks.md\` from the task graph. Record \`serial_reason\` and \`maxParallelReason\`; split tasks with more than six \`target_files\` or add a concrete \`scope_reason\`; use task-graph allowlist derive/check/apply where applicable.
...
\`implementation-plan.md\` は \`design.md\` から導き、対象ファイル、期待結果、検証コマンド、依存関係、並行可能な作業、競合を記録する。\`artifacts/agents/task-graph.json\` は \`implementation-plan.md\` から導き、\`tasks.md\` は task graph から導く。serial task には \`serial_reason\`、single-worker limit には \`maxParallelReason\` を記録する。6 個を超える \`target_files\` は task を分割するか具体的な \`scope_reason\` を記録し、L3 allowlist は task graph から derive/check/apply する。
...
يجب اشتقاق \`implementation-plan.md\` من \`design.md\` مع الملفات المستهدفة والنتائج المتوقعة وأوامر التحقق والاعتماديات والعمل القابل للتوازي والتعارضات؛ ويجب اشتقاق \`artifacts/agents/task-graph.json\` من \`implementation-plan.md\`؛ ويجب اشتقاق \`tasks.md\` من task graph. سجل \`serial_reason\` للمهام المتسلسلة و\`maxParallelReason\` لحد worker واحد؛ قسّم task التي تتجاوز ستة \`target_files\` أو أضف \`scope_reason\` واضحاً؛ واستخدم derive/check/apply لقائمة L3.
