...
9. 多 worker 执行服从 \`runtimeAdapter.selected.nativeSubagent\`；只并行派发安全 batch。capability 缺失、target 不匹配或过期时必须阻断，不得启动 agent CLI 或退回当前 controller。所有 native wait 单次最多 60 秒；每个 live child 在 \`heartbeatDueAt\` 前续租，完成一个立即持久化一个，并在每轮 poll 后重新 tick。capacity 未知时 implementation 批次最多两个，但不降低安全 review 并行度。
...
9. Execute \`runtimeAdapter.selected.nativeSubagent\` and parallelize only the safe batch. Missing, mismatched, or expired capability blocks dispatch with no agent CLI or current-controller fallback. Every native wait returns within 60 seconds; refresh each live child before \`heartbeatDueAt\`, persist each finished result immediately, and re-tick after every poll. Unknown capacity caps implementation batches at two without reducing safe review parallelism.
...
9. \`runtimeAdapter.selected.nativeSubagent\` を実行し、安全な batch だけを並列 dispatch する。capability がない、target mismatch、または期限切れの場合は block し、agent CLI や current controller に fallback しない。すべての native wait は 60 秒以内に戻り、各 live child を \`heartbeatDueAt\` 前に更新し、完了結果を直ちに保存して poll ごとに再 tick する。capacity 不明時は implementation batch を 2 件までに制限するが、安全な review 並列性は維持する。
...
多 worker 执行服从 \`runtimeAdapter.selected.nativeSubagent\`；只在所选 native adapter 支持并行时启动整个安全 batch。capability 缺失、过期或 target 不匹配时必须阻断，不能回退到 Orca、agent CLI 或 current controller。所有 native wait 单次最多 60 秒；每个 live child 在 \`heartbeatDueAt\` 前续租，完成一个立即持久化一个，并在每轮 poll 后重新 tick。capacity 未知时 implementation 批次最多两个，但不降低安全 review 并行度。
...
multi-worker execution は \`runtimeAdapter.selected.nativeSubagent\` に従う。選択された native adapter が parallel work をサポートするときだけ safe batch を並列起動する。capability がない、期限切れ、または target 不一致の場合は block し、Orca、agent CLI、current controller に fallback しない。すべての native wait は 60 秒以内に戻り、各 live child を \`heartbeatDueAt\` 前に更新し、完了結果を直ちに保存して poll ごとに再 tick する。capacity 不明時は implementation batch を 2 件までに制限するが、安全な review 並列性は維持する。
