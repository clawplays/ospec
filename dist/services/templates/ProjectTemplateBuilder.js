...
goal 使用 controller 模式时，不要停在 Loop 初始化或让用户手工运行命令；门禁就绪后，当前 IDE 主 AI 必须立即执行 \`ospec loop run [change] --once --json\`，通过原生 subagent API 派发全部 actions，等待并记录 evidence，然后无需用户再次提示就继续 tick，直到完成或遇到真实门禁。
...
For a goal in controller mode, do not stop at Loop initialization or ask the user to run commands. Once gates are ready, the current IDE AI must immediately run \`ospec loop run [change] --once --json\`, dispatch every action through the native subagent API, wait and record evidence, then keep ticking without another user prompt until done or genuinely blocked.
...
controller mode の goal では Loop 初期化で停止したり、ユーザーにコマンド実行を依頼したりしない。gate が ready になったら current IDE AI が直ちに \`ospec loop run [change] --once --json\` を実行し、native subagent API で全 action を dispatch、結果と evidence を記録し、完了または実際の blocker までユーザーの追加 prompt なしで tick を続ける。
...
عند استخدام goal في controller mode، لا تتوقف عند تهيئة Loop ولا تطلب من المستخدم تشغيل الأوامر. بعد جاهزية gates، يجب على IDE AI الحالي تشغيل \`ospec loop run [change] --once --json\` فورا، وتوزيع كل actions عبر native subagent API، وانتظار النتائج وتسجيل evidence، ثم متابعة tick من دون prompt إضافي حتى الاكتمال أو وجود blocker حقيقي.
