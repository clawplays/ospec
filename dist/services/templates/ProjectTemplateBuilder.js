...
22. task graph 派生后执行一次独立 combined planning review；若为 \`NEEDS_CHANGES\`，最多允许一次整体规划修复和一次 fresh re-review，重复失败必须稳定停止。task review、最终 combined review 和验证仍然保留。
...
22. After task graph derivation, run one independent combined planning review. A \`NEEDS_CHANGES\` decision permits at most one grouped planning repair and one fresh re-review; a repeated failure must stop stably. Task review, final combined review, and verification remain required.
...
22. task graph 導出後に独立した combined planning review を 1 回実行する。\`NEEDS_CHANGES\` では grouped planning repair と fresh re-review を各 1 回だけ許可し、再失敗は安定して停止する。task review、final combined review、verification は引き続き必須。
...
22. بعد اشتقاق task graph شغّل combined planning review مستقلة واحدة. يسمح قرار \`NEEDS_CHANGES\` بإصلاح تخطيط مجمّع واحد وfresh re-review واحدة فقط، ثم يجب التوقف بثبات عند تكرار الفشل. تبقى task review وfinal combined review وverification مطلوبة.
...
task graph 派生后执行一次独立 combined planning review；若为 \`NEEDS_CHANGES\`，最多允许一次整体规划修复和一次 fresh re-review，重复失败必须稳定停止。task review、最终 combined review 和验证仍然保留。
...
After task graph derivation, run one independent combined planning review. A \`NEEDS_CHANGES\` decision permits at most one grouped planning repair and one fresh re-review; repeated failure stops stably. Task review, final combined review, and verification remain required.
...
task graph 導出後に独立した combined planning review を 1 回実行する。\`NEEDS_CHANGES\` では grouped planning repair と fresh re-review を各 1 回だけ許可し、再失敗は安定して停止する。task review、final combined review、verification は引き続き必須。
...
بعد اشتقاق task graph شغّل combined planning review مستقلة واحدة. يسمح قرار \`NEEDS_CHANGES\` بإصلاح تخطيط مجمّع واحد وfresh re-review واحدة فقط، ثم يجب التوقف بثبات عند تكرار الفشل. تبقى task review وfinal combined review وverification مطلوبة.
