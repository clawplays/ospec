...
5. \`Announce-Before-Act\`：绝不静默执行——宣告当前 skill 与阶段、即将运行的 \`ospec execute ...\` 命令与产物、所选 model-native subagent adapter、worker 数量、当前 session capability，以及具体原生机制（Claude Task、Codex/GPT \`spawn_agent\`、Gemini \`@generalist\`、OpenCode \`@mention\` 或已注册的 native primitive）和阻塞门禁。
...
5. 用户明确点名的浏览器、E2E、skill 或工具验证是硬性要求；不得自动选择会移除该要求的较弱验证方案。能力不可用时必须报告阻断并保持该要求未完成。
5. 当 change 必须等待用户选择后才能继续时，用 \`ospec execute decision [changes/active/<change>] --id <id> --question "..." --option id:label:影响 --option id:label:影响 [--recommended id] [--required]\` 写入持久 decision gate，向用户展示 decision report 的 \`Chat Prompt\` 或 \`artifacts/agents/decisions/index.md\`，再用 \`ospec execute decision [changes/active/<change>] --id <id> --select <option-id> --answered-by user\` 记录用户选择。
6. \`implementation-plan.md\` 必须从 \`design.md\` 推导；design 和 plan 的 inline preflight 通过后，才能从 plan 推导 \`artifacts/agents/task-graph.json\`，再从 task graph 推导 \`tasks.md\`。生成的 \`parallelizable: false\` task 必须包含 \`serial_reason\`，显式限制单 worker 时记录 \`maxParallelReason\`；超过六个 \`target_files\` 的 task 必须拆分或填写具体的 \`scope_reason\`。普通 red test、对应生产实现和 green/refactor 证据放在同一个原子 task；需要额外边界时可从任务图 derive/check/apply 可选白名单。
6. 进入已有 OSpec 项目时，用 \`ospec session [path]\` 写入 \`.ospec/session-brief.json\` 和 \`.ospec/session-brief.md\`，记录 active work 的 \`change\` 或 \`goal\` profile、queued change、queue-run、cache fingerprint 和按 profile 生成的安全下一步命令；经典 Change 直接读取五个核心文件，只有 Goal 使用 \`ospec execute bootstrap\`。
...
6. Goal 用 \`ospec execute status --brief\` 和当前 packet 定位下一步；经典 Change 用 \`ospec progress\`、顶层 \`ospec verify\` 和 \`ospec finalize\`，不得进入 task graph controller。
7. 开始或恢复单个 active Goal 时，用 \`ospec execute bootstrap [changes/active/<goal>]\` 写入带 project session brief snapshot 的 \`artifacts/agents/bootstrap.json\` 和 \`artifacts/agents/bootstrap.md\`，然后按其中的下一步安全动作继续；已有 active dispatch 时，bootstrap 会推荐对应的 \`ospec execute launch ... --task ...\` 命令。
...
8. change 需要在 agent、工具、worktree、shell 或人工操作者之间交接时，用 \`ospec execute handoff [changes/active/<change>] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot|shell|generic]\` 写入 \`artifacts/agents/handoff.json\` 和 \`artifacts/agents/handoff.md\`；该命令只记录 project session brief snapshot、目标工具映射和安全规则，不会启动 worker 或编辑源码。
8. 派生 task graph 前，依次运行 \`ospec execute preflight [changes/active/<change>] --stage design\` 和 \`--stage plan\`，在 \`artifacts/agents/planning-preflights/\` 下生成带 project session brief snapshot 的确定性 inline preflight packet，并写入 \`artifacts/reviews/design-review.md\` 与 \`artifacts/reviews/implementation-plan-review.md\`；两步通过后再派生或刷新 task graph，任何阶段都不启动 reviewer child。
8. 派发 worker 前，用 \`ospec execute workspace [changes/active/<change>]\` 写入 \`artifacts/agents/workspace-status.json\` 和 \`artifacts/agents/workspace-status.md\`；恢复已有 Goal 时，只接受非 \`PENDING\` 任务目标文件和当前哈希校验通过的 \`ospec update\` 证明所归属的脏路径，其余脏路径仍为 \`needs_isolation\`。
...
9. dispatch 后用 \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]\` 写入 \`launch-plan.md\`；\`runtimeAdapter\` 只接受当前、target 匹配的模型原生 subagent capability，并给出 native primitive。
9. 多 worker 执行服从 \`runtimeAdapter.selected.nativeSubagent\`；只并行派发安全 batch。capability 缺失、target 不匹配或过期时必须阻断，不得启动 agent CLI 或退回当前 controller。所有 native wait 单次最多 60 秒；这只是 controller poll 上限，不是 child 运行上限。每个 live child 在 \`heartbeatDueAt\` 前续租，完成后用 action 的 \`loop finalize\` 原子提交证据和结果，并在每轮 poll 后重新 tick。capacity 未知时 implementation 使用默认并发 3，但不降低安全 review 并行度；harness 能可靠报告当前更大 child 容量时可绑定到当前 controller session，并相应提高 \`maxParallel\`，不得猜测或复用过期容量。
9. agent CLI 执行已移除；\`execute orchestrate\`、\`launch --run --command\`、\`review --run --command\` 与 \`loop watch\` 都会在启动进程或创建 run artifact 前失败。修复 native work 后用 \`ospec execute retry\` 重新派发。
10. controller Loop 管理 Goal 时，每个 worker task 和最终整体 review 都由 \`ospec loop tick [changes/active/<change>]\` 派发，使 \`artifacts/agents/review-dispatches/\` 绑定真实 executor provenance；只有非 controller 流程才直接运行 \`ospec execute review\`。task 级决策写入 \`artifacts/reviews/tasks/<task-id>/review.md\`。
...
12. \`ospec session\`、\`ospec execute bootstrap\`、\`handoff\`、\`preflight\`、\`workspace\`、plan 模式 \`worktree\`、\`finish\`、\`dispatch\`、\`launch\`、\`collect\`、\`retry\`、\`complete\`、\`review\`、\`feedback\`、\`debug\`、\`tdd\`、\`verify\` 与 \`sync\` 只更新 OSpec artifacts；除 \`workspace\`、\`worktree\` 与 \`finish\` 会读取 git 状态外，不直接编辑项目 source files。worker 只能由当前模型 controller 通过 \`runtimeAdapter.selected.nativeSubagent\` 启动；OSpec 不执行 agent CLI。
...
18. 运行最新项目验证命令后，用 \`ospec execute verify [changes/active/<change>] --command "..." --status PASSED --exit-code 0\` 记录 \`artifacts/agents/verification-evidence.json\`；PASSED 必须显式绑定退出码 0，不要只用聊天摘要声称完成。
...
22. task graph 派生后执行一次独立 combined planning review；若为 \`NEEDS_CHANGES\`，最多允许一次整体规划修复和一次差量复审；未改动规划内容的执行器失败会重新武装修复额度，修复后 findings 全部不高于 medium 时确定性通过为 \`APPROVED_WITH_CONCERNS\`，语义层面重复失败必须稳定停止。task review、最终 combined review 和验证仍然保留。
23. worker/reviewer 使用逻辑 model profile，并按实际 dispatch target（包括 launch override）解析；requested/configured 与 provider-observed model 分开记录，没有 provider/usage evidence 时 observed model 保持 unknown。命令执行器通过 \`OSPEC_USAGE_FILE\` 自动归集 usage，\`--usage-file\` 保留为手工入口；指标必须区分 complete、partial、missing，不能把未上报显示成零。
...
5. \`Announce-Before-Act\`: never run the workflow silently — announce the OSpec skill/stage, command and artifact, selected model-native subagent adapter, worker count, current session capability, actual native mechanism (Claude Task, Codex/GPT \`spawn_agent\`, Gemini \`@generalist\`, OpenCode \`@mention\`, or the registered native primitive), and any blocking gate.
...
5. A user-requested browser, E2E, skill, or tool verification surface is mandatory; never auto-select a weaker verification option that removes it. Report a capability blocker and leave it incomplete when the surface is unavailable.
5. When the change must pause for a user choice, record a durable gate with \`ospec execute decision [changes/active/<change>] --id <id> --question "..." --option id:label:impact --option id:label:impact [--recommended id] [--required]\`, present the decision report \`Chat Prompt\` or \`artifacts/agents/decisions/index.md\`, then record the selected option with \`ospec execute decision [changes/active/<change>] --id <id> --select <option-id> --answered-by user\`.
6. Derive \`implementation-plan.md\` from \`design.md\`; after design and plan inline preflights pass, derive \`artifacts/agents/task-graph.json\` from the plan and \`tasks.md\` from the graph. Generated \`parallelizable: false\` tasks require \`serial_reason\`, and an explicit single-worker limit requires \`maxParallelReason\`; split tasks with more than six \`target_files\` or add a concrete \`scope_reason\`. Keep ordinary red tests with their production implementation and green/refactor evidence in one atomic task; derive/check/apply an optional task-graph allowlist only when an extra boundary is requested.
6. When entering an existing OSpec project, use \`ospec session [path]\` to write \`.ospec/session-brief.json\` and \`.ospec/session-brief.md\` with the active work's \`change\` or \`goal\` profile, queued change, queue-run, cache fingerprint, and profile-aware next commands; a classic Change reads its five core files directly, while only a Goal uses \`ospec execute bootstrap\`.
...
6. A Goal uses \`ospec execute status --brief\` and the current packet to locate the next action; a classic Change uses \`ospec progress\`, top-level \`ospec verify\`, and \`ospec finalize\` and must not enter the task graph controller.
...
8. When a change moves between agents, tools, worktrees, shells, or human operators, use \`ospec execute handoff [changes/active/<change>] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot|shell|generic]\` to write \`artifacts/agents/handoff.json\` and \`artifacts/agents/handoff.md\`; this records the project session brief snapshot, target tool mapping, and safety rules only and does not launch workers or edit source files.
8. Before deriving the task graph, run \`ospec execute preflight [changes/active/<change>] --stage design\`, then \`--stage plan\`, to create deterministic inline preflight packets under \`artifacts/agents/planning-preflights/\` and approvals at \`artifacts/reviews/design-review.md\` and \`artifacts/reviews/implementation-plan-review.md\`. Derive or refresh the task graph only after both pass; neither stage launches a reviewer child.
8. Before worker handoff, use \`ospec execute workspace [changes/active/<change>]\` to write \`artifacts/agents/workspace-status.json\` and \`artifacts/agents/workspace-status.md\`; when resuming an existing Goal, exact dirty paths owned by non-\`PENDING\` task targets and current hash-verified \`ospec update\` provenance are accepted, while every other dirty path remains \`needs_isolation\`.
...
9. After dispatch, use \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]\` to write \`launch-plan.md\`. Its \`runtimeAdapter\` accepts only a current, target-bound model-native subagent capability and exposes the native primitive.
9. Execute \`runtimeAdapter.selected.nativeSubagent\` and parallelize only the safe batch. Missing, mismatched, or expired capability blocks dispatch with no agent CLI or current-controller fallback. Every native wait returns within 60 seconds; this bounds one controller poll, not child runtime. Refresh each live child before \`heartbeatDueAt\`, commit finished evidence and results with the action's \`loop finalize\`, and re-tick after every poll. Unknown capacity uses the default implementation concurrency of three without reducing safe review parallelism. A harness that authoritatively knows a larger current child capacity may bind it to the active controller session and raise \`maxParallel\` accordingly; never guess or reuse stale capacity.
9. Agent CLI execution is removed: \`execute orchestrate\`, \`launch --run --command\`, \`review --run --command\`, and \`loop watch\` fail before process launch or run-artifact creation. Use \`ospec execute retry\` for corrected native work.
10. When a controller Loop owns the Goal, issue every task and final review with \`ospec loop tick [changes/active/<change>]\` so \`artifacts/agents/review-dispatches/\` is bound to real executor provenance; use \`ospec execute review\` directly only outside a controller Loop. Task decisions remain under \`artifacts/reviews/tasks/<task-id>/review.md\`.
...
12. \`ospec session\`, \`ospec execute bootstrap\`, \`handoff\`, \`preflight\`, \`workspace\`, plan-mode \`worktree\`, \`finish\`, \`dispatch\`, \`launch\`, \`collect\`, \`retry\`, \`complete\`, \`review\`, \`feedback\`, \`debug\`, \`tdd\`, \`verify\`, and \`sync\` update OSpec artifacts only; except for \`workspace\`, \`worktree\`, and \`finish\` reading git state, they do not edit project source files directly. The current model controller dispatches workers only through \`runtimeAdapter.selected.nativeSubagent\`; OSpec does not execute agent CLIs.
...
18. After running fresh project verification commands, use \`ospec execute verify [changes/active/<change>] --command "..." --status PASSED --exit-code 0\` to record \`artifacts/agents/verification-evidence.json\`; PASSED requires an explicit zero exit code, and a chat summary is not evidence.
...
22. After task graph derivation, run one independent combined planning review. A \`NEEDS_CHANGES\` decision permits one grouped planning repair and at most one delta-scoped re-review; a no-edit executor failure re-arms the repair, all-medium-or-lower findings settle deterministically as \`APPROVED_WITH_CONCERNS\` after the repair, and a repeated semantic failure must stop stably. Task review, final combined review, and verification remain required.
23. Workers and reviewers use logical model profiles resolved against the actual dispatch target, including launch overrides. Keep requested/configured and provider-observed models separate; without provider/usage evidence the observed model remains unknown. Command runners ingest \`OSPEC_USAGE_FILE\` automatically while \`--usage-file\` remains a manual input; metrics distinguish complete, partial, and missing coverage rather than treating absent counters as zero.
...
5. \`Announce-Before-Act\`: ワークフローを黙って実行しない。skill・stage、\`ospec execute ...\` command と artifact、model-native subagent adapter、worker 数、current session capability、native mechanism（Claude Task、Codex/GPT \`spawn_agent\`、Gemini \`@generalist\`、OpenCode \`@mention\`、または登録済み native primitive）、blocking gate を伝える。
...
5. ユーザーが指定した browser、E2E、skill、tool の検証は必須であり、それを外す弱い検証案を自動選択しない。利用不能なら blocker を報告し、未完了のまま残す。
5. change がユーザー選択を待つ必要がある場合は \`ospec execute decision [changes/active/<change>] --id <id> --question "..." --option id:label:impact --option id:label:impact [--recommended id] [--required]\` で durable decision gate を記録し、decision report の \`Chat Prompt\` または \`artifacts/agents/decisions/index.md\` を提示してから、\`ospec execute decision [changes/active/<change>] --id <id> --select <option-id> --answered-by user\` で回答を記録する。
6. \`implementation-plan.md\` は \`design.md\` から導く。design と plan の inline preflight 通過後に plan から \`artifacts/agents/task-graph.json\`、graph から \`tasks.md\` を導く。生成した \`parallelizable: false\` task には \`serial_reason\`、明示的な single-worker limit には \`maxParallelReason\` が必要。6 個を超える \`target_files\` は分割するか具体的な \`scope_reason\` を記録する。通常の red test、production implementation、green/refactor evidence は 1 つの atomic task にまとめ、追加境界が必要な場合だけ optional allowlist を derive/check/apply する。
6. 既存の OSpec project に入るときは \`ospec session [path]\` で \`.ospec/session-brief.json\` と \`.ospec/session-brief.md\` を書き、active work の \`change\` / \`goal\` profile、queued change、queue-run、cache fingerprint、profile-aware な次の command を記録する。classic Change は 5 つの core file を直接読み、Goal だけが \`ospec execute bootstrap\` を使う。
...
6. Goal は \`ospec execute status --brief\` と現在の packet で次の action を特定する。classic Change は \`ospec progress\`、top-level \`ospec verify\`、\`ospec finalize\` を使い、task graph controller に入らない。
7. one active Goal を開始または再開するときは、\`ospec execute bootstrap [changes/active/<goal>]\` で project session brief snapshot を含む \`artifacts/agents/bootstrap.json\` と \`artifacts/agents/bootstrap.md\` を書き、そこにある次の安全な action に従う。active dispatch がある場合、bootstrap は対応する \`ospec execute launch ... --task ...\` command を推奨する。
...
8. change を agent、tool、worktree、shell、human operator の間で引き渡すときは、\`ospec execute handoff [changes/active/<change>] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot|shell|generic]\` で \`artifacts/agents/handoff.json\` と \`artifacts/agents/handoff.md\` を書く。このコマンドは project session brief snapshot、target tool mapping、safety rules のみを記録し、worker 起動や source file 編集は行わない。
8. task graph 導出前に \`ospec execute preflight [changes/active/<change>] --stage design\`、続いて \`--stage plan\` を実行し、\`artifacts/agents/planning-preflights/\` 配下に deterministic inline preflight packet、\`artifacts/reviews/design-review.md\` と \`artifacts/reviews/implementation-plan-review.md\` に approval を作成する。両方の通過後に task graph を導出または更新し、reviewer child は起動しない。
8. worker handoff の前に \`ospec execute workspace [changes/active/<change>]\` で \`artifacts/agents/workspace-status.json\` と \`artifacts/agents/workspace-status.md\` を書く。既存 Goal の再開では、非 \`PENDING\` task の target file または現在のハッシュ検証済み \`ospec update\` provenance に正確に属する dirty path だけを許可し、それ以外は \`needs_isolation\` のままにする。
...
9. dispatch 後は \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]\` で launch plan を書く。\`runtimeAdapter\` は current かつ target-bound な model-native subagent capability のみを受け入れる。
9. \`runtimeAdapter.selected.nativeSubagent\` を実行し、安全な batch だけを並列 dispatch する。capability がない、target mismatch、または期限切れの場合は block し、agent CLI や current controller に fallback しない。すべての native wait は 60 秒以内に戻るが、これは controller poll の上限で child runtime の上限ではない。各 live child を \`heartbeatDueAt\` 前に更新し、完了時は action の \`loop finalize\` で evidence と result を保存して poll ごとに再 tick する。capacity 不明時の implementation は既定の並列数 3 を使用し、安全な review 並列性は維持する。harness が現在のより大きな child capacity を確実に把握できる場合は active controller session に結び付け、必要に応じて \`maxParallel\` を引き上げられるが、capacity を推測したり古い値を再利用してはならない。
9. agent CLI execution は削除された。\`execute orchestrate\`、\`launch --run --command\`、\`review --run --command\`、\`loop watch\` は process 起動や run artifact 作成の前に失敗する。native work の再実行には \`ospec execute retry\` を使う。
10. controller Loop が Goal を所有する場合、task/final review は \`ospec loop tick [changes/active/<change>]\` で発行し、\`artifacts/agents/review-dispatches/\` を実 executor provenance に関連付ける。\`ospec execute review\` を直接使うのは non-controller workflow のみとする。
10. task graph 完了後の final review も controller Loop の次の \`ospec loop tick\` で発行する。\`ospec execute review\` を直接使うのは non-controller workflow のみとする。
...
12. \`ospec session\`、\`ospec execute bootstrap\`、\`handoff\`、\`preflight\`、\`workspace\`、plan-mode \`worktree\`、\`finish\`、\`dispatch\`、\`launch\`、\`collect\`、\`retry\`、\`complete\`、\`review\`、\`feedback\`、\`debug\`、\`tdd\`、\`verify\`、\`sync\` は OSpec artifacts のみを更新する。\`workspace\`、\`worktree\`、\`finish\` が git state を読む場合を除き、project source files は直接編集しない。current model controller は \`runtimeAdapter.selected.nativeSubagent\` だけで worker を dispatch し、OSpec は agent CLI を実行しない。
...
18. fresh project verification commands を実行した後、\`ospec execute verify [changes/active/<change>] --command "..." --status PASSED --exit-code 0\` で \`artifacts/agents/verification-evidence.json\` を記録する。PASSED には明示的な exit code 0 が必要で、chat summary だけで完了を主張しない。
...
22. task graph 導出後に独立した combined planning review を 1 回実行する。\`NEEDS_CHANGES\` では grouped planning repair 1 回と delta re-review 最大 1 回を許可する。planning 内容を変更しない executor 失敗は許容量を再アームし、findings がすべて medium 以下なら repair 後に \`APPROVED_WITH_CONCERNS\` として決定論的に確定し、意味的な再失敗は安定して停止する。task review、final combined review、verification は引き続き必須。
23. worker/reviewer の logical model profile は launch override を含む actual dispatch target に対して解決する。requested/configured model と provider-observed model を分離し、provider/usage evidence がなければ observed model は unknown とする。command runner は \`OSPEC_USAGE_FILE\` を自動集計し、\`--usage-file\` は手動入力として残す。
...
5. \`Announce-Before-Act\`: لا تشغّل workflow بصمت؛ أعلن skill/stage وcommand وartifact وmodel-native subagent adapter وعدد workers وcurrent session capability والـ native mechanism الفعلي (Claude Task أو Codex/GPT \`spawn_agent\` أو Gemini \`@generalist\` أو OpenCode \`@mention\` أو native primitive مسجل) وأي blocking gate.
...
5. أي تحقق يطلبه المستخدم صراحة عبر browser أو E2E أو skill أو tool إلزامي؛ لا تختَر تلقائياً تحققاً أضعف يحذفه، وسجّل blocker واتركه غير مكتمل عند عدم توفره.
5. عندما يجب أن ينتظر change اختيار المستخدم، سجّل durable decision gate عبر \`ospec execute decision [changes/active/<change>] --id <id> --question "..." --option id:label:impact --option id:label:impact [--recommended id] [--required]\`، واعرض \`Chat Prompt\` من decision report أو \`artifacts/agents/decisions/index.md\`، ثم سجّل الإجابة عبر \`ospec execute decision [changes/active/<change>] --id <id> --select <option-id> --answered-by user\`.
6. اشتق \`implementation-plan.md\` من \`design.md\`، وبعد نجاح inline preflight للتصميم والخطة اشتق \`artifacts/agents/task-graph.json\` من الخطة و\`tasks.md\` من graph. تتطلب المهام ذات \`parallelizable: false\` قيمة \`serial_reason\`، ويتطلب حد worker واحد \`maxParallelReason\`؛ قسّم task التي تتجاوز ستة \`target_files\` أو أضف \`scope_reason\` واضحاً. اجمع red test العادي وproduction implementation ودليل green/refactor في atomic task واحدة، واستخدم allowlist اختيارية فقط عند طلب حد إضافي.
6. عند الدخول إلى مشروع OSpec موجود، استخدم \`ospec session [path]\` لكتابة \`.ospec/session-brief.json\` و\`.ospec/session-brief.md\` مع profile ‏\`change\` أو \`goal\` للعمل النشط وqueued change وqueue-run وcache fingerprint والأوامر التالية الموافقة للـ profile؛ يقرأ Change الكلاسيكي ملفاته الخمسة مباشرة، ويستخدم Goal فقط \`ospec execute bootstrap\`.
...
6. يستخدم Goal ‏\`ospec execute status --brief\` وpacket الحالي لتحديد الإجراء التالي؛ يستخدم Change الكلاسيكي \`ospec progress\` و\`ospec verify\` على المستوى الأعلى و\`ospec finalize\` ولا يدخل task graph controller.
7. عند بدء أو استئناف active Goal واحد، استخدم \`ospec execute bootstrap [changes/active/<goal>]\` لكتابة \`artifacts/agents/bootstrap.json\` و\`artifacts/agents/bootstrap.md\` مع project session brief snapshot، ثم اتبع الإجراء الآمن التالي المسجل فيه؛ عند وجود active dispatch، يوصي bootstrap بأمر \`ospec execute launch ... --task ...\` المطابق.
...
8. قبل اشتقاق task graph، شغّل \`ospec execute preflight [changes/active/<change>] --stage design\` ثم \`--stage plan\` لإنشاء deterministic inline preflight packets داخل \`artifacts/agents/planning-preflights/\` وapprovals في \`artifacts/reviews/design-review.md\` و\`artifacts/reviews/implementation-plan-review.md\`. اشتق أو حدّث task graph بعد نجاح المرحلتين فقط، ولا تشغّل reviewer child.
8. قبل handoff إلى worker، استخدم \`ospec execute workspace [changes/active/<change>]\` لكتابة \`artifacts/agents/workspace-status.json\` و\`artifacts/agents/workspace-status.md\`؛ عند استئناف Goal قائم، تُقبل فقط المسارات المتسخة التابعة لأهداف task غير \`PENDING\` أو لإثبات \`ospec update\` حالي متحقق من الهاش، وتظل أي مسارات أخرى \`needs_isolation\`.
...
9. بعد dispatch، استخدم \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]\` لكتابة launch plan؛ يقبل \`runtimeAdapter\` فقط model-native subagent capability حالية ومرتبطة بالـ target.
9. نفّذ \`runtimeAdapter.selected.nativeSubagent\` وشغّل safe batch فقط بالتوازي. عند غياب capability أو target mismatch أو انتهاء الصلاحية يجب block من دون agent CLI أو fallback إلى current controller. يعيد كل native wait التحكم خلال 60 ثانية؛ هذا حد controller poll واحدة وليس حد child runtime. حدّث كل child حي قبل \`heartbeatDueAt\`، واستخدم \`loop finalize\` الصادر مع action لحفظ evidence وresult عند الاكتمال، ثم أعد tick بعد كل poll. عند غياب capacity يستخدم implementation التوازي الافتراضي وهو ثلاث مهام من دون تقليل توازي review الآمن. إذا كان harness يعرف بثقة capacity حالية أكبر للـ child فيمكنه ربطها بجلسة controller النشطة ورفع \`maxParallel\` وفقاً لذلك؛ لا تخمّن capacity ولا تعِد استخدام قيمة قديمة.
9. أزيل agent CLI execution. تفشل \`execute orchestrate\` و\`launch --run --command\` و\`review --run --command\` و\`loop watch\` قبل تشغيل process أو إنشاء run artifact. استخدم \`ospec execute retry\` لإعادة native work.
10. عندما تكون الـ Goal مملوكة لـ controller Loop، أصدر task/final reviews عبر \`ospec loop tick [changes/active/<change>]\` لربط \`artifacts/agents/review-dispatches/\` بـ executor provenance الحقيقي؛ استخدم \`ospec execute review\` مباشرةً فقط خارج controller Loop.
...
12. تحدّث أوامر OSpec المذكورة artifacts فقط ولا تعدّل project source files مباشرة. يشغّل current model controller الـ workers فقط عبر \`runtimeAdapter.selected.nativeSubagent\`؛ ولا يشغّل OSpec agent CLI.
...
18. بعد تشغيل project verification commands حديثة، استخدم \`ospec execute verify [changes/active/<change>] --command "..." --status PASSED --exit-code 0\` لتسجيل \`artifacts/agents/verification-evidence.json\`؛ تتطلب PASSED رمز خروج 0 صريحا، ولا يكفي ملخص المحادثة.
...
22. بعد اشتقاق task graph شغّل combined planning review مستقلة واحدة. يسمح قرار \`NEEDS_CHANGES\` بإصلاح تخطيط مجمّع واحد وإعادة مراجعة تفاضلية واحدة كحد أقصى؛ يعيد فشل المنفّذ دون تعديل التخطيط تسليح الحصة، وتُقبل النتائج التي لا تتجاوز medium حتمياً كـ \`APPROVED_WITH_CONCERNS\` بعد الإصلاح، ثم يجب التوقف بثبات عند تكرار الفشل الدلالي. تبقى task review وfinal combined review وverification مطلوبة.
23. يحل worker/reviewer logical model profile حسب actual dispatch target بما في ذلك launch override. افصل requested/configured model عن provider-observed model؛ بدون provider/usage evidence يبقى observed model غير معروف. يجمع command runner \`OSPEC_USAGE_FILE\` تلقائيا ويبقى \`--usage-file\` للإدخال اليدوي.
...
\`workflow_profile_id: goal\` 使用完整流程，要求 \`design.md\`、\`implementation-plan.md\`、\`artifacts/agents/task-graph.json\`、planning preflights、task/final reviews、worker status 和 verification evidence。
进入已有 OSpec 项目时，用 \`ospec session [path]\` 写入 \`.ospec/session-brief.json\` 和 \`.ospec/session-brief.md\`；该项目入口简报记录 active work 的 \`change\` 或 \`goal\` profile、queued change、queue-run、cache fingerprint 和按 profile 生成的安全下一步命令。经典 Change 直接读取五个核心文件，只有 Goal 使用 \`ospec execute bootstrap\`。
...
\`Announce-Before-Act\`：绝不静默执行——宣告当前 skill 与阶段、即将运行的 \`ospec execute ...\` 命令与产物、所选 model-native subagent adapter、worker 数量、当前 session capability 以及被哪个门禁阻塞。
...
用户明确点名的浏览器、E2E、skill 或工具验证是硬性要求；不得自动降级，能力不可用时必须报告阻断并保持未完成。
...
\`implementation-plan.md\` 必须从 \`design.md\` 推导；design/plan inline preflight 通过后，从 plan 推导 \`artifacts/agents/task-graph.json\`，再从 graph 推导 \`tasks.md\`。串行 task 写 \`serial_reason\`，单 worker 限制写 \`maxParallelReason\`；超过六个 \`target_files\` 的 task 必须拆分或填写具体的 \`scope_reason\`。普通 red test、对应生产实现和 green/refactor 证据放在同一个原子 task；仅在需要额外边界时从任务图 derive/check/apply 可选白名单。
开始或恢复单个 active Goal 时，用 \`ospec execute bootstrap [changes/active/<goal>]\` 写入带 project session brief snapshot 的 \`artifacts/agents/bootstrap.json\` 和 \`artifacts/agents/bootstrap.md\`，然后按其中的下一步安全动作继续；经典 Change 改用 \`ospec progress\`、顶层 \`ospec verify\` 和 \`ospec finalize\`，不得进入 task graph controller。
...
change 需要在 agent、工具、worktree、shell 或人工操作者之间交接时，用 \`ospec execute handoff [changes/active/<change>] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot|shell|generic]\` 写入 \`artifacts/agents/handoff.json\` 和 \`artifacts/agents/handoff.md\`；该命令只记录 project session brief snapshot、目标工具映射和安全规则，不会启动 worker 或编辑源码。
派生 task graph 前，依次运行 \`ospec execute preflight [changes/active/<change>] --stage design\` 和 \`--stage plan\`，在 \`artifacts/agents/planning-preflights/\` 下生成确定性 inline preflight packet，并写入 \`artifacts/reviews/design-review.md\` 与 \`artifacts/reviews/implementation-plan-review.md\`；两步通过后再派生或刷新 task graph，不启动 reviewer child。
派发 worker 前，用 \`ospec execute workspace [changes/active/<change>]\` 写入 \`artifacts/agents/workspace-status.json\`（\`workspace-status.json\`）和 \`artifacts/agents/workspace-status.md\`；恢复已有 Goal 时，只接受非 \`PENDING\` 任务目标文件和当前哈希校验通过的 \`ospec update\` 证明所归属的脏路径，其余脏路径仍为 \`needs_isolation\`。
...
dispatch 后用 \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]\` 写入 \`launch-plan.md\`；只有与 target 匹配、仍在有效期内的当前 session capability 才能选择 \`runtimeAdapter.selected.nativeSubagent\`。
多 worker 执行服从 \`runtimeAdapter.selected.nativeSubagent\`；只在所选 native adapter 支持并行时启动整个安全 batch。capability 缺失、过期或 target 不匹配时必须阻断，不能回退到 Orca、agent CLI 或 current controller。所有 native wait 单次最多 60 秒；这只是 controller poll 上限，不是 child 运行上限。每个 live child 在 \`heartbeatDueAt\` 前续租，完成后用 action 的 \`loop finalize\` 原子提交证据和结果，并在每轮 poll 后重新 tick。capacity 未知时 implementation 使用默认并发 3，但不降低安全 review 并行度；harness 能可靠报告当前更大 child 容量时可绑定到当前 controller session，并相应提高 \`maxParallel\`，不得猜测或复用过期容量。
goal 使用 controller 模式时，不要停在 Loop 初始化或让用户手工运行命令；门禁就绪后，当前 IDE 主 AI 必须立即执行 \`ospec loop run [change] --once --json\`，通过每个 action 的 \`runtimeAdapter.selected\` 执行并记录 heartbeat/result evidence，然后无需用户再次提示就继续 tick，直到完成或遇到真实门禁。
\`execute orchestrate\`、\`launch --run --command\`、\`review --run --command\` 与 \`loop watch\` 已移除并在任何进程或 run artifact 创建前失败。修复 blocked、needs-context 或 failed work 后，用 \`ospec execute retry\` 重新派发；已完成任务必须显式 \`--force\` 才能 retry。
controller Loop 管理 Goal 时，每个 task 和最终整体 review 都由 \`ospec loop tick [changes/active/<change>]\` 派发，使 \`artifacts/agents/review-dispatches/\` 绑定真实 executor provenance；只有非 controller 流程才直接运行 \`ospec execute review\`。task 决策写入 \`artifacts/reviews/tasks/<task-id>/review.md\`。
review packet 必须交给新的 model-native reviewer subagent；OSpec 不运行本地 reviewer CLI。reviewer 完成后写回 review decision 与 evidence。
...
\`ospec session\`、\`ospec execute bootstrap\`、\`handoff\`、\`preflight\`、\`workspace\`、plan 模式 \`worktree\`、\`finish\`、\`dispatch\`、\`launch\`、\`collect\`、\`retry\`、\`complete\`、\`review\`、\`feedback\`、\`debug\`、\`tdd\`、\`verify\` 与 \`sync\` 只更新 OSpec artifacts；除 \`workspace\`、\`worktree\` 与 \`finish\` 会读取 git 状态外，不直接编辑项目 source files。current model controller 只通过 \`runtimeAdapter.selected.nativeSubagent\` 派发 worker；OSpec 不执行 agent CLI。
...
运行最新项目验证命令后，必须用 \`ospec execute verify [changes/active/<change>] --command "..." --status PASSED --exit-code 0\` 记录 \`artifacts/agents/verification-evidence.json\`。
...
task graph 派生后执行一次独立 combined planning review；若为 \`NEEDS_CHANGES\`，最多允许一次整体规划修复和一次差量复审；未改动规划内容的执行器失败会重新武装修复额度，修复后 findings 全部不高于 medium 时确定性通过为 \`APPROVED_WITH_CONCERNS\`，语义层面重复失败必须稳定停止。task review、最终 combined review 和验证仍然保留。
worker/reviewer packet 必须记录按实际 target 解析的逻辑 model profile，并区分 requested/configured 与 provider-observed model；没有 provider/usage evidence 时 observed model 保持 unknown。命令执行器自动归集 \`OSPEC_USAGE_FILE\`，并报告 complete/partial/missing 覆盖率。
...
For \`workflow_profile_id: goal\`, use the full workflow and require \`design.md\`, \`implementation-plan.md\`, \`artifacts/agents/task-graph.json\`, planning preflights, task/final reviews, worker status, and verification evidence.
When entering an existing OSpec project, use \`ospec session [path]\` to write \`.ospec/session-brief.json\` and \`.ospec/session-brief.md\`; this project entry brief records the active work's \`change\` or \`goal\` profile, queued change, queue-run, cache fingerprint, and profile-aware next commands. A classic Change reads its five core files directly, while only a Goal uses \`ospec execute bootstrap\`.
...
A user-requested browser, E2E, skill, or tool verification surface is mandatory; never silently downgrade it, and report a blocker when it is unavailable.
When the change must pause for a user choice, record a durable gate with \`ospec execute decision [changes/active/<change>] --id <id> --question "..." --option id:label:impact --option id:label:impact [--recommended id] [--required]\`, present the decision report \`Chat Prompt\` or \`artifacts/agents/decisions/index.md\`, then record the selected option with \`ospec execute decision [changes/active/<change>] --id <id> --select <option-id> --answered-by user\`.
Derive \`implementation-plan.md\` from \`design.md\`; after design/plan inline preflights pass, derive \`artifacts/agents/task-graph.json\` from the plan and \`tasks.md\` from the graph. Record \`serial_reason\` and \`maxParallelReason\`; split tasks with more than six \`target_files\` or add a concrete \`scope_reason\`. Keep ordinary red tests with their production implementation and green/refactor evidence in one atomic task unless the test harness is independently reusable; use task-graph allowlist derive/check/apply where applicable.
When starting or resuming one active Goal, use \`ospec execute bootstrap [changes/active/<goal>]\` to write \`artifacts/agents/bootstrap.json\` and \`artifacts/agents/bootstrap.md\` with the project session brief snapshot, then follow its next safe action. A classic Change uses \`ospec progress\`, top-level \`ospec verify\`, and \`ospec finalize\` and must not enter the task graph controller.
...
Before deriving the task graph, run \`ospec execute preflight [changes/active/<change>] --stage design\`, then \`--stage plan\`, to create deterministic inline preflight packets under \`artifacts/agents/planning-preflights/\` and approvals at \`artifacts/reviews/design-review.md\` and \`artifacts/reviews/implementation-plan-review.md\`. Derive or refresh the task graph only after both pass; neither stage launches a reviewer child.
Before worker handoff, use \`ospec execute workspace [changes/active/<change>]\` to write \`artifacts/agents/workspace-status.json\` (\`workspace-status.json\`) and \`artifacts/agents/workspace-status.md\`; when resuming an existing Goal, exact dirty paths owned by non-\`PENDING\` task targets and current hash-verified \`ospec update\` provenance are accepted, while every other dirty path remains \`needs_isolation\`.
...
After dispatch, use \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]\` to write \`launch-plan.md\`. Only a current session capability that matches the target may select \`runtimeAdapter.selected.nativeSubagent\`.
Multi-worker execution follows \`runtimeAdapter.selected.nativeSubagent\`; launch the safe batch in parallel only when the selected native adapter supports it. Missing, expired, or target-mismatched capability must block; never fall back to Orca, an agent CLI, or the current controller. Every native wait returns within 60 seconds; this bounds one controller poll, not child runtime. Refresh each live child before \`heartbeatDueAt\`, commit finished evidence and results with the action's \`loop finalize\`, and re-tick after every poll. Unknown capacity uses the default implementation concurrency of three without reducing safe review parallelism. A harness that authoritatively knows a larger current child capacity may bind it to the active controller session and raise \`maxParallel\` accordingly; never guess or reuse stale capacity.
For a goal in controller mode, do not stop at Loop initialization or ask the user to run commands. Once gates are ready, the current IDE AI must immediately run \`ospec loop run [change] --once --json\`, execute every action through its \`runtimeAdapter.selected\`, persist heartbeat and result evidence, then keep ticking without another user prompt until done or genuinely blocked.
\`execute orchestrate\`, \`launch --run --command\`, \`review --run --command\`, and \`loop watch\` are removed and fail before any process or run artifact is created. Use \`ospec execute retry\` to reopen corrected blocked, needs-context, or failed work; completed tasks require \`--force\`.
When a controller Loop owns the Goal, issue every task and final review with \`ospec loop tick [changes/active/<change>]\` so \`artifacts/agents/review-dispatches/\` is bound to real executor provenance; use \`ospec execute review\` directly only outside a controller Loop. Task decisions remain under \`artifacts/reviews/tasks/<task-id>/review.md\`.
Dispatch each review packet to a fresh model-native reviewer subagent; OSpec never runs a local reviewer CLI. Record the review decision and evidence after the reviewer completes.
...
\`ospec session\`, \`ospec execute bootstrap\`, \`handoff\`, \`preflight\`, \`workspace\`, plan-mode \`worktree\`, \`finish\`, \`dispatch\`, \`launch\`, \`collect\`, \`retry\`, \`complete\`, \`review\`, \`feedback\`, \`debug\`, \`tdd\`, \`verify\`, and \`sync\` update OSpec artifacts only; except for \`workspace\`, \`worktree\`, and \`finish\` reading git state, they do not edit project source files directly. The controller dispatches workers only through the selected model-native subagent adapter.
...
After running fresh project verification commands, record \`artifacts/agents/verification-evidence.json\` with \`ospec execute verify [changes/active/<change>] --command "..." --status PASSED --exit-code 0\`.
...
After task graph derivation, run one independent combined planning review. A \`NEEDS_CHANGES\` decision permits one grouped planning repair and at most one delta-scoped re-review; a no-edit executor failure re-arms the repair, all-medium-or-lower findings settle deterministically as \`APPROVED_WITH_CONCERNS\` after the repair, and repeated semantic failure stops stably. Task review, final combined review, and verification remain required.
Worker/reviewer packets resolve the logical model profile against the actual target and separate requested/configured from provider-observed model; absent provider/usage evidence keeps observed model unknown. Command runners ingest \`OSPEC_USAGE_FILE\` automatically and report complete/partial/missing coverage.
...
\`workflow_profile_id: goal\` では full workflow を使い、\`design.md\`、\`implementation-plan.md\`、\`artifacts/agents/task-graph.json\`、planning preflights、task/final reviews、worker status、verification evidence を要求する。
既存の OSpec project に入るときは \`ospec session [path]\` で \`.ospec/session-brief.json\` と \`.ospec/session-brief.md\` を書く。この project entry brief は active work の \`change\` / \`goal\` profile、queued change、queue-run、cache fingerprint、profile-aware な次の command を記録する。classic Change は 5 つの core file を直接読み、Goal だけが \`ospec execute bootstrap\` を使う。
...
\`Announce-Before-Act\`: ワークフローを黙って実行しない。skill・段階、コマンドと生成物、選択された model-native subagent adapter、worker 数、current session capability、blocking gate を伝える。
...
\`implementation-plan.md\` は \`design.md\` から導く。design/plan inline preflight 通過後に plan から \`artifacts/agents/task-graph.json\`、graph から \`tasks.md\` を導く。serial task には \`serial_reason\`、single-worker limit には \`maxParallelReason\` を記録する。6 個を超える \`target_files\` は分割するか具体的な \`scope_reason\` を記録する。通常の red test、production implementation、green/refactor evidence は 1 つの atomic task にまとめ、追加境界が必要な場合だけ optional allowlist を derive/check/apply する。
one active Goal を開始または再開するときは、\`ospec execute bootstrap [changes/active/<goal>]\` で project session brief snapshot を含む \`artifacts/agents/bootstrap.json\` と \`artifacts/agents/bootstrap.md\` を書き、そこにある次の安全な action に従う。classic Change は \`ospec progress\`、top-level \`ospec verify\`、\`ospec finalize\` を使い、task graph controller に入らない。
...
task graph 導出前に \`ospec execute preflight [changes/active/<change>] --stage design\`、続いて \`--stage plan\` を実行し、\`artifacts/agents/planning-preflights/\` 配下に deterministic inline preflight packet、\`artifacts/reviews/design-review.md\` と \`artifacts/reviews/implementation-plan-review.md\` に approval を作成する。両方の通過後に task graph を導出または更新し、reviewer child は起動しない。
worker handoff の前に \`ospec execute workspace [changes/active/<change>]\` で \`artifacts/agents/workspace-status.json\`（\`workspace-status.json\`）と \`artifacts/agents/workspace-status.md\` を書く。既存 Goal の再開では、非 \`PENDING\` task の target file または現在のハッシュ検証済み \`ospec update\` provenance に正確に属する dirty path だけを許可し、それ以外は \`needs_isolation\` のままにする。
...
dispatch 後は \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]\` で \`artifacts/agents/launch-plan.md\` を書く。target と一致し、有効期限内の current session capability だけが \`runtimeAdapter.selected.nativeSubagent\` を選択できる。
multi-worker execution は \`runtimeAdapter.selected.nativeSubagent\` に従う。選択された native adapter が parallel work をサポートするときだけ safe batch を並列起動する。capability がない、期限切れ、または target 不一致の場合は block し、Orca、agent CLI、current controller に fallback しない。すべての native wait は 60 秒以内に戻るが、これは controller poll の上限で child runtime の上限ではない。各 live child を \`heartbeatDueAt\` 前に更新し、完了時は action の \`loop finalize\` で evidence と result を保存して poll ごとに再 tick する。capacity 不明時の implementation は既定の並列数 3 を使用し、安全な review 並列性は維持する。harness が現在のより大きな child capacity を確実に把握できる場合は active controller session に結び付け、必要に応じて \`maxParallel\` を引き上げられるが、capacity を推測したり古い値を再利用してはならない。
controller mode の goal では Loop 初期化で停止したり、ユーザーにコマンド実行を依頼したりしない。gate が ready になったら current IDE AI が直ちに \`ospec loop run [change] --once --json\` を実行し、各 action をその \`runtimeAdapter.selected\` で実行し、heartbeat・result evidence を記録して、完了または実際の blocker までユーザーの追加 prompt なしで tick を続ける。
\`execute orchestrate\`、\`launch --run --command\`、\`review --run --command\`、\`loop watch\` は削除され、process や run artifact を作る前に失敗する。blocked/needs-context/failed work の修正後は \`ospec execute retry\` で再 dispatch する。完了済み task は \`--force\` が必要。
controller Loop が Goal を所有する場合、task/final review は \`ospec loop tick [changes/active/<change>]\` で発行し、\`artifacts/agents/review-dispatches/\` を実 executor provenance に関連付ける。\`ospec execute review\` を直接使うのは non-controller workflow のみとする。
task graph 完了後の final review も controller Loop の次の \`ospec loop tick\` で発行する。\`ospec execute review\` を直接使うのは non-controller workflow のみとする。
review packet は fresh model-native reviewer subagent に dispatch する。OSpec は local reviewer CLI を実行しない。reviewer 完了後に decision と evidence を記録する。
...
\`ospec session\`、\`ospec execute bootstrap\`、\`handoff\`、\`preflight\`、\`workspace\`、plan-mode \`worktree\`、\`finish\`、\`dispatch\`、\`launch\`、\`collect\`、\`retry\`、\`complete\`、\`review\`、\`feedback\`、\`debug\`、\`tdd\`、\`verify\`、\`sync\` は OSpec artifacts のみを更新する。\`workspace\`、\`worktree\`、\`finish\` が git state を読む場合を除き、project source files は直接編集しない。controller は選択された model-native subagent adapter だけで worker を dispatch する。
...
fresh project verification commands を実行した後、\`ospec execute verify [changes/active/<change>] --command "..." --status PASSED --exit-code 0\` で \`artifacts/agents/verification-evidence.json\` を記録する。
...
task graph 導出後に独立した combined planning review を 1 回実行する。\`NEEDS_CHANGES\` では grouped planning repair 1 回と delta re-review 最大 1 回を許可する。planning 内容を変更しない executor 失敗は許容量を再アームし、findings がすべて medium 以下なら repair 後に \`APPROVED_WITH_CONCERNS\` として決定論的に確定し、意味的な再失敗は安定して停止する。task review、final combined review、verification は引き続き必須。
worker/reviewer packet は actual target に対する logical model profile を記録し、requested/configured と provider-observed model を分離する。provider/usage evidence がなければ observed model は unknown。command runner は \`OSPEC_USAGE_FILE\` を自動集計する。
...
عند \`workflow_profile_id: goal\` استخدم المسار الكامل واطلب \`design.md\` و\`implementation-plan.md\` و\`artifacts/agents/task-graph.json\` وplanning preflights وtask/final reviews وworker status وverification evidence.
عند الدخول إلى مشروع OSpec موجود، استخدم \`ospec session [path]\` لكتابة \`.ospec/session-brief.json\` و\`.ospec/session-brief.md\`؛ يسجل هذا project entry brief ‏profile ‏\`change\` أو \`goal\` للعمل النشط وqueued change وqueue-run وcache fingerprint والأوامر التالية الموافقة للـ profile. يقرأ Change الكلاسيكي ملفاته الخمسة مباشرة، ويستخدم Goal فقط \`ospec execute bootstrap\`.
...
\`Announce-Before-Act\`: لا تُشغّل سير العمل بصمت — أعلن أي skill ومرحلة، وأي أمر \`ospec execute ...\` ستشغّله وما يكتبه، وmodel-native subagent adapter المختار، وعدد workers، وcurrent session capability، وأي بوابة تحجب التقدم.
...
اشتق \`implementation-plan.md\` من \`design.md\`، وبعد نجاح design/plan inline preflight اشتق \`artifacts/agents/task-graph.json\` من الخطة و\`tasks.md\` من graph. سجل \`serial_reason\` و\`maxParallelReason\`؛ قسّم task التي تتجاوز ستة \`target_files\` أو أضف \`scope_reason\` واضحاً. اجمع red test العادي وproduction implementation ودليل green/refactor في atomic task واحدة، واستخدم allowlist اختيارية فقط عند طلب حد إضافي.
عند بدء أو استئناف active Goal واحد، استخدم \`ospec execute bootstrap [changes/active/<goal>]\` لكتابة \`artifacts/agents/bootstrap.json\` و\`artifacts/agents/bootstrap.md\` مع project session brief snapshot، ثم اتبع الإجراء الآمن التالي المسجل فيه. يستخدم Change الكلاسيكي \`ospec progress\` و\`ospec verify\` على المستوى الأعلى و\`ospec finalize\` ولا يدخل task graph controller.
...
قبل اشتقاق task graph، شغّل \`ospec execute preflight [changes/active/<change>] --stage design\` ثم \`--stage plan\` لإنشاء deterministic inline preflight packets داخل \`artifacts/agents/planning-preflights/\` وapprovals في \`artifacts/reviews/design-review.md\` و\`artifacts/reviews/implementation-plan-review.md\`. اشتق أو حدّث task graph بعد نجاح المرحلتين فقط، ولا تشغّل reviewer child.
قبل handoff إلى worker، استخدم \`ospec execute workspace [changes/active/<change>]\` لكتابة \`artifacts/agents/workspace-status.json\` (\`workspace-status.json\`) و\`artifacts/agents/workspace-status.md\`؛ عند استئناف Goal قائم، تُقبل فقط المسارات المتسخة التابعة لأهداف task غير \`PENDING\` أو لإثبات \`ospec update\` حالي متحقق من الهاش، وتظل أي مسارات أخرى \`needs_isolation\`.
...
بعد dispatch، استخدم \`ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]\` لكتابة \`artifacts/agents/launch-plan.md\`؛ لا يختار \`runtimeAdapter.selected.nativeSubagent\` إلا current session capability مطابقة للـ target وغير منتهية.
يتبع multi-worker execution القيمة \`runtimeAdapter.selected.nativeSubagent\`، ولا يشغّل safe batch بالتوازي إلا عندما يدعم native adapter المختار ذلك. يجب block عند غياب capability أو انتهائها أو عدم مطابقة target، من دون fallback إلى Orca أو agent CLI أو current controller. يعيد كل native wait التحكم خلال 60 ثانية؛ هذا حد controller poll واحدة وليس حد child runtime. حدّث كل child حي قبل \`heartbeatDueAt\`، واستخدم \`loop finalize\` الصادر مع action لحفظ evidence وresult عند الاكتمال، ثم أعد tick بعد كل poll. عند غياب capacity يستخدم implementation التوازي الافتراضي وهو ثلاث مهام من دون تقليل توازي review الآمن. إذا كان harness يعرف بثقة capacity حالية أكبر للـ child فيمكنه ربطها بجلسة controller النشطة ورفع \`maxParallel\` وفقاً لذلك؛ لا تخمّن capacity ولا تعِد استخدام قيمة قديمة.
عند استخدام goal في controller mode، لا تتوقف عند تهيئة Loop ولا تطلب من المستخدم تشغيل الأوامر. بعد جاهزية gates، يجب على IDE AI الحالي تشغيل \`ospec loop run [change] --once --json\` فورا، وتنفيذ كل action عبر \`runtimeAdapter.selected\` الخاص به، وتسجيل heartbeat وresult evidence، ثم متابعة tick من دون prompt إضافي حتى الاكتمال أو وجود blocker حقيقي.
...
عندما تكون الـ Goal مملوكة لـ controller Loop، أصدر task/final reviews عبر \`ospec loop tick [changes/active/<change>]\` لربط \`artifacts/agents/review-dispatches/\` بـ executor provenance الحقيقي؛ استخدم \`ospec execute review\` مباشرةً فقط خارج controller Loop.
أرسل review packet إلى fresh model-native reviewer subagent؛ لا يشغّل OSpec local reviewer CLI. بعد اكتمال reviewer سجّل decision وevidence.
...
بعد تشغيل project verification commands حديثة، سجل \`artifacts/agents/verification-evidence.json\` باستخدام \`ospec execute verify [changes/active/<change>] --command "..." --status PASSED --exit-code 0\`.
...
بعد اشتقاق task graph شغّل combined planning review مستقلة واحدة. يسمح قرار \`NEEDS_CHANGES\` بإصلاح تخطيط مجمّع واحد وإعادة مراجعة تفاضلية واحدة كحد أقصى؛ يعيد فشل المنفّذ دون تعديل التخطيط تسليح الحصة، وتُقبل النتائج التي لا تتجاوز medium حتمياً كـ \`APPROVED_WITH_CONCERNS\` بعد الإصلاح، ثم يجب التوقف بثبات عند تكرار الفشل الدلالي. تبقى task review وfinal combined review وverification مطلوبة.
تحل worker/reviewer packets logical model profile حسب actual target وتفصل requested/configured عن provider-observed model؛ بدون provider/usage evidence يبقى observed model غير معروف. يجمع command runner \`OSPEC_USAGE_FILE\` تلقائيا.
