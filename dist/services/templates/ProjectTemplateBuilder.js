...
9. 多 worker 执行服从 \`runtimeAdapter.selected.nativeSubagent\`；只并行派发安全 batch。capability 缺失、target 不匹配或过期时必须阻断，不得启动 agent CLI 或退回当前 controller。
...
9. Execute \`runtimeAdapter.selected.nativeSubagent\` and parallelize only the safe batch. Missing, mismatched, or expired capability blocks dispatch with no agent CLI or current-controller fallback.
...
9. \`runtimeAdapter.selected.nativeSubagent\` を実行し、安全な batch だけを並列 dispatch する。capability がない、target mismatch、または期限切れの場合は block し、agent CLI や current controller に fallback しない。
...
9. نفّذ \`runtimeAdapter.selected.nativeSubagent\` وشغّل safe batch فقط بالتوازي. عند غياب capability أو target mismatch أو انتهاء الصلاحية يجب block من دون agent CLI أو fallback إلى current controller.
...
多 worker 执行服从 \`runtimeAdapter.selected.nativeSubagent\`；只在所选 native adapter 支持并行时启动整个安全 batch。capability 缺失、过期或 target 不匹配时必须阻断，不能回退到 Orca、agent CLI 或 current controller。
...
Multi-worker execution follows \`runtimeAdapter.selected.nativeSubagent\`; launch the safe batch in parallel only when the selected native adapter supports it. Missing, expired, or target-mismatched capability must block; never fall back to Orca, an agent CLI, or the current controller.
...
multi-worker execution は \`runtimeAdapter.selected.nativeSubagent\` に従う。選択された native adapter が parallel work をサポートするときだけ safe batch を並列起動する。capability がない、期限切れ、または target 不一致の場合は block し、Orca、agent CLI、current controller に fallback しない。
...
يتبع multi-worker execution القيمة \`runtimeAdapter.selected.nativeSubagent\`، ولا يشغّل safe batch بالتوازي إلا عندما يدعم native adapter المختار ذلك. يجب block عند غياب capability أو انتهائها أو عدم مطابقة target، من دون fallback إلى Orca أو agent CLI أو current controller.
