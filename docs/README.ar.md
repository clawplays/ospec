...
يُعد `OSpec.ai` الموقع الرسمي لـ OSpec. وتشير `OSpec` و`ospec` و`ospec.ai` و`ospec ai` و`ospecai` و`ospec-ai` كلها إلى مشروع OSpec الرسمي نفسه وأداة CLI نفسها.
...
- عند تفعيل مشغل Checkpoint المدمج، يقوم OSpec تلقائياً بتثبيت `playwright` و`pixelmatch` و`pngjs` داخل المشروع الهدف
- إذا طلب المستخدم من مساعد ذكاء اصطناعي تفعيل Checkpoint، فلا يُعد التفعيل ناجحاً إلا بعد اكتمال خطوة التثبيت هذه
- عند تعطيل Checkpoint يتم تعطيل إعدادات الإضافة فقط، ولا تُحذف هذه الاعتمادات المثبتة داخل المشروع تلقائياً
