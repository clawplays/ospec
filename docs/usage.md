...
- Use `ospec execute doc-review [changes/active/<change>] [--stage design|plan]` before implementation dispatch to create the reviewer packet. For a specialist review, the IDE controller must record `--claim-executor <child-id>` immediately after spawning the native child and `--complete-executor <child-id>` after waiting for its Markdown and structured findings. Approval is bound to that child, controller session, document hash, and review timestamp; design review must be approved before plan review.
