...
- Use `ospec execute doc-review [changes/active/<change>] [--stage design|plan]` before implementation dispatch. Specialist packets include an independent `runtimeAdapter`; claim the real Orca terminal, native child, or CLI process id immediately, then complete it after Markdown and structured findings are ready. Native reviews additionally bind the controller session; design review must pass before plan review.
...
- Use `ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run]` after dispatch to write the agent launch plan. In addition to target-native instructions, the artifact records `runtimeAdapter`: verified Orca workspace, current harness-native capability, available target CLI, and serial generic-controller fallback in that order. Orca is never selected from a process name alone. OSpec writes `artifacts/agents/launch-plan.json` and `artifacts/agents/launch-plan.md`, requires one active dispatch and ready workspace status, and does not start workers or run shell commands by itself.
- Default multi-worker execution follows `runtimeAdapter.selected`: create a parallel-safe batch with `ospec execute dispatch`, inspect `launch-plan.md`, then start one worker per safe packet when the selected adapter supports parallel execution. A preferred adapter probe failure falls through automatically; ordinary work may run serially in the current controller, while independent review still requires an independent adapter. Record each result with `ospec execute complete`.
- Use `ospec execute orchestrate [changes/active/<change>] --command "..."` when the selected adapter or recorded fallback order chooses explicit CLI orchestration. OSpec runs only the adapter-supported batch, writes `artifacts/agents/orchestration-runs/`, and collects each run back into the task graph.
- Use `ospec execute launch ... --run --command "..."` when the selected target-CLI adapter needs a single-worker runner or an earlier adapter fails before claiming ownership. OSpec captures stdout/stderr, timeout metadata, and the exit code under `artifacts/agents/worker-runs/`; then use `ospec execute collect ...` to record task completion.
...
- `ospec execute bootstrap`, `handoff`, `doc-review`, `status`, `next`, and `route` are read-only with respect to project source files; the artifact commands write only their documented state. The controller launches workers through the selected runtime adapter. Shell execution occurs only through explicit worktree, runner, orchestration, selected external-adapter, or configured CLI-driven `ospec loop watch` execution.
