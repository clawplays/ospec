...
- عند تفعيل مشغل Checkpoint / Playwright المدمج افتراضياً، يجب على OSpec أن يثبت `playwright` و`pixelmatch` و`pngjs` تلقائياً داخل المشروع الهدف
- إذا طلب المستخدم عبر محادثة الذكاء الاصطناعي أو عبر مساعد ذكاء اصطناعي "تفعيل إضافة Checkpoint"، فإن خطوة التثبيت التلقائي هذه جزء إلزامي من مسار التفعيل
- إذا فشل تثبيت الاعتمادات، فيجب أن يفشل `enable checkpoint` بدلاً من ترك المشروع في حالة تفعيل جزئية
- عند تعطيل الإضافة يتم فقط إيقاف `.skillrc.plugins.checkpoint` ولا يتم حذف اعتمادات المشروع هذه تلقائياً
...
1. `base_url`
2. التأكد من وجود الاعتمادات المثبتة تلقائياً داخل المشروع الهدف
3. `routes.yaml`
4. `flows.yaml`
5. حالة تسجيل الدخول أو نص تسجيل الدخول
6. أمر بدء التشغيل وفحص الجاهزية (إذا كان لا يمكن الوصول إلى المشروع مباشرة)
...
### 2. الاعتمادات التي يتم تثبيتها تلقائياً

يعتمد المشغل المدمج الافتراضي على هذه الحزم داخل المشروع الهدف:

- `playwright`
- `pixelmatch`
- `pngjs`

لم تعد هذه الاعتمادات تُحمَل داخل الحزمة الرئيسية لـ OSpec، بل تُثبَّت داخل المشروع الهدف عند تفعيل Checkpoint.

وهذا يعني:

- تبقى الحزمة الرئيسية أخف
- فقط المشاريع التي تفعّل Checkpoint فعلاً تتحمل تكلفة هذه الاعتمادات
- مسارات التفعيل عبر AI chat يجب أن تشمل التثبيت، وليس مجرد كتابة الإعدادات
- تعطيل Checkpoint لاحقاً لا يجب أن يزيل هذه الاعتمادات تلقائياً
...
### 6. أمر بدء التشغيل وفحص الجاهزية
...
3. التأكد من أن المشروع الهدف يحتوي الآن على `playwright` و`pixelmatch` و`pngjs`
4. تكوين `routes.yaml` و `flows.yaml` وحالة تسجيل الدخول وبيئة التشغيل
5. تشغيل فحص التشخيص: `ospec plugins doctor checkpoint .`
6. إنشاء تغيير يتطلب تحققاً آلياً
7. تنفيذ الفحص: `ospec plugins run checkpoint <change-path>`
8. بعد اجتياز الفحص، قم بتنفيذ `ospec verify` و `ospec finalize`
...
4. عند تمكين `checkpoint` باستخدام المشغل المدمج الافتراضي، يجب تثبيت `playwright` و`pixelmatch` و`pngjs` تلقائياً داخل المشروع الهدف.
5. في مسارات AI chat أو المسارات المدعومة بالذكاء الاصطناعي، فإن عبارة "تفعيل إضافة Checkpoint" تشمل خطوة تثبيت هذه الاعتمادات؛ وإذا لم تكتمل فلا يُعد التفعيل ناجحاً.
6. تعطيل `checkpoint` يعطل الإضافة فقط، ولا يزيل اعتمادات المشروع التي تم تثبيتها مسبقاً.
7. يدعم النظام حالياً `base_url` واحداً فقط، دون التبديل بين بيئات متعددة.
8. `checkpoint` هي إضافة بوابة آلية؛ ولا تقدم أوامر موافقة / رفض يدوية.
9. إذا كان المشروع مفعلاً لـ `stitch` وكان التغيير الحالي يفعل `stitch_design_review` ، فإن `checkpoint` تعطي الأولوية لإعادة استخدام تصميم الأساس المصدر من قبل Stitch.
10. إذا لم يكن المشروع مفعلاً لـ `stitch` ، فإن `checkpoint` تستخدم لقطات الشاشة الأساسية ومتطلبات النص داخل المستودع كأساس لفحص التصميم.
11. نظراً لأن المشاريع قد لا تمتلك قدرة بدء تشغيل محلية، يجب أن تدعم `checkpoint` أوامر بدء التشغيل المخصصة؛ وإذا كان المشروع يمكنه استخدام `docker compose` ، فمن الأفضل البدء من خلاله.
12. حالة تسجيل الدخول هي قدرة قياسية مدعومة، وتعتمد افتراضياً على `storageState` أو نص تسجيل دخول مخصص.
13. تتكون فحوصات صحة البيانات من جزأين: توكيدات Playwright للصفحة/الواجهة، وأوامر توكيد الحالة النهائية للخلفية المخصصة للمشروع.
14. تسمح `checkpoint` بتفعيل قدرات محددة فقط بناءً على أعلام التغيير، بدلاً من تشغيل فحوصات كاملة لكل تغيير.
15. ترتيب التنفيذ الموصى به هو `ui_review` أولاً، ثم `flow_check` .
...
- يتطلب `enable checkpoint` وجود `base_url`
- يجب على `enable checkpoint` باستخدام المشغل المدمج أن يثبت `playwright` و`pixelmatch` و`pngjs` تلقائياً داخل المشروع الهدف
- في تدفقات AI chat لا يكتمل تفعيل Checkpoint قبل اكتمال خطوة التثبيت التلقائي هذه
- `disable checkpoint` لا يزيل اعتمادات المشروع
- لا توفر `checkpoint` أوامر `approve` أو `reject` لأنها بوابة آلية
