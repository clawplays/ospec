...
There is no agent CLI execution mode. `loop watch` and `execute orchestrate` no longer exist as commands, and `launch --run --command` / `review --run --command` reject those flags before creating a process or run artifact.
...
Design and implementation-plan checks are deterministic inline preflights. They consume no reviewer token reservation and never create a native child. After task graph derivation, one independent combined planning reviewer checks requirements, architecture, task boundaries, dependencies, and verification coverage. `NEEDS_CHANGES` permits one grouped planning repair and at most one delta-scoped re-review. An executor failure that changed no planning content re-arms the repair instead of consuming the allowance; after a completed repair, findings that were all medium or lower settle deterministically as `APPROVED_WITH_CONCERNS` without an AI re-review, while higher-severity findings get one delta-scoped re-review that receives the round-one findings, the repair diff, and finality guidance. Planning approvals are invalidated only by semantic planning changes (documents, task semantics), never by execution bookkeeping such as task status, checklist ticks, or commits; repeated semantic failure blocks stably. Task and final review repair convergence remains controlled by `--continue-while-progressing` and the configured repair thresholds.
