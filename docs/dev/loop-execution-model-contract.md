# Loop Engineering - Integrated Execution-Model Contract (1.8.5)
...
Task-review repair has a dedicated round guard because status transitions can represent real workflow movement while still repeating the same implementation/review cycle. `maxTaskRepairRounds` defaults to two automatic rounds per task. Manual retries and worker-status retries do not consume this counter. Raising the limit is an explicit user-authorized action after unresolved findings are inspected.

Grouped final-review repair has an independent guard. `maxFinalRepairRounds` defaults to two repair waves, counted from durable `repair-waves` records. A `BLOCKED` final review is a gate that requires blocker resolution; only `NEEDS_CHANGES` may create a grouped repair wave. Raising `--max-final-repair-rounds` requires the same explicit user authorization after unresolved final findings are inspected. Corrupt or unreadable repair-wave history fails closed; a change with no repair-wave history starts at zero.
...
Default specialist limits are two completed rounds per stage, 30 minutes per stage, and two no-progress outcomes. No-progress accounting is bound to the current review-context hash and resets after authoritative inputs change; the 30-minute stage deadline remains hard and cannot be reset by editing documents. Cache reuse, pending-dispatch reuse, deterministic preflight, heartbeat, lease reclaim, and crash recovery of the same dispatch do not consume a round. STOP, pause, expiry, total Goal budgets, stage budgets, and token reservations prevent a new dispatch but do not prevent the current claimed reviewer from heartbeating or completing.
...
Scheduling diagnostics distinguish registered parallel support, reported harness capacity (known or unknown), configured maximum, graph-safe candidates, token-funded candidates, and the effective emitted count. When a parallel native adapter does not report capacity, an implementation batch is conservatively capped at two actions; task-review parallelism remains bounded by the normal graph/configuration/token rules. A known harness capacity replaces the conservative unknown-capacity guard. An explicit `maxParallel` is never raised automatically. `maxParallel=1` should carry `maxParallelReason`, and new task graphs should give every `parallelizable:false` task a `serial_reason`; legacy graphs remain readable. A 1.8.5 task with more than six `target_files` must be split or carry a non-empty `scope_reason` that explains its atomic bounded ownership. Reasons document intent but never bypass dependency, path, or conflict checks. Task-graph-derived L3 paths are enforced as exact files, and command cwd is always exact; only explicitly manual directory allowlists retain subtree semantics.
...
- 1.8.5 bounded native wait fields, task `scope_reason`, and scheduling diagnostics are additive. 1.8.4, 1.8.3, and 1.8.2 artifacts remain readable; missing capacity stays explicitly unknown and activates only the conservative implementation-batch cap.
