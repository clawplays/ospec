...
- When entering an existing OSpec project, run `ospec session [path]` to write `.ospec/session-brief.json` and `.ospec/session-brief.md` with the active work's `change` or `goal` profile, queued change, queue-run, cache fingerprint, and profile-aware next commands. A classic Change reads its five core files directly; only a Goal uses `ospec execute bootstrap`. Use `ospec session hook [path]` only to write optional harness startup hook artifacts with the same profile-aware routing and decision/plugin gate sources
...
- Use `ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]` after dispatch to write the launch plan. Its `runtimeAdapter` accepts only a current, target-bound model-native subagent capability and exposes the native primitive
...
- For an integrated goal loop in controller mode, do not stop after initialization or ask the user to run Loop commands. Run `ospec loop run [change] --once --compact-json`, execute every emitted action through its `runtimeAdapter.selected.nativeSubagent`, claim each child once, run `ospec loop poll` between bounded waits, and run a full tick only when poll reports `tickNow=true` — all without another user prompt; do not paste the whole goal into each worker
- `IDE-CONTROLLER-AUTO-DISPATCH`: every Goal uses the same executable fast quality workflow. The IDE AI owns tick -> execute all `actions[]` through model-native subagents -> claim each child once -> `loop poll` between waits -> full tick when `tickNow=true`. An empty `actions[]` with `pending` is observation state: wait/poll only and never relaunch it
