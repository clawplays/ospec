...
- `Announce-Before-Act`: never run the workflow silently. Announce in one line which OSpec skill you are using and the current stage; which `ospec execute ...` command you are about to run and the artifact it writes; how many native subagents you dispatch and via which mechanism (`Task` for Claude Code, `spawn_agent` plus the harness wait lifecycle for Codex/GPT, `@generalist` for Gemini, `@mention` for OpenCode; close only if exposed); and which gate is blocking when progress stops
...
- Before deriving or dispatching implementation tasks, use `ospec execute doc-review [changes/active/<change>] [--stage design|plan]` to create the document reviewer packet. For specialist mode, spawn one native reviewer child, immediately record `--claim-executor <child-id>`, wait for its Markdown and structured findings, then record `--complete-executor <child-id>`; design review must validate before implementation plan review.
...
- Use `ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]` after dispatch to write the native agent launch plan; it tells the controlling AI how to use the current harness agent mechanism (`spawn_agent` plus the harness wait lifecycle for Codex/GPT, Task for Claude Code, `@generalist` for Gemini, and `@mention` for OpenCode; close only if exposed). Use `--json` when an adapter needs the machine-readable launch artifact on stdout. It does not start workers or edit source files by itself
...
- For an integrated goal loop in controller mode, do not stop after goal/Loop initialization or ask the user to run Loop commands. Continue in the same IDE AI session; when gates are ready, run `ospec loop run [change] --once --json`, immediately launch every emitted action item as one fresh native subagent batch through the IDE API, wait for results, record the required durable evidence, and tick again without another user prompt; do not paste the whole goal into each worker
- `IDE-CONTROLLER-AUTO-DISPATCH`: L1 is report-only. For a user-approved L2/L3 goal, the IDE AI owns tick -> dispatch all `actions[]` -> wait -> evidence -> immediate tick; an empty `actions[]` with `pending` is observation only and must never be relaunched. Do not use `loop watch` when the IDE native subagent API is available
...
- `ospec execute doc-review` records artifacts only and does not launch reviewers, run shell commands, sync worker status, or edit source files; native subagent dispatch is done by the current AI harness. Shell execution occurs only through explicit worktree create/cleanup, fallback `launch --run --command`, `review --run --command`, fallback `orchestrate` with a command template, or a configured CLI-driven `ospec loop watch`
...
- Workers and reviewers use logical model profiles; actual models resolve from `.skillrc.workflow.model_profiles` by target, otherwise the harness default is explicit.
