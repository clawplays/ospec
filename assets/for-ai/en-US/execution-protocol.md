...
- Present decision options using the harness-native question UI when available (Claude Code: `AskUserQuestion`) before recording the selection; in Claude Code, `ospec session hook --target claude --apply` installs hooks that re-affirm this contract and hard-block subagent dispatch while a required decision is pending
