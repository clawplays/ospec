...
- قبل اشتقاق implementation tasks أو dispatch لها، استخدم `ospec execute doc-review [changes/active/<change>] [--stage design|plan]` لإنشاء document reviewer packet. في specialist mode شغّل native reviewer child وسجّل فورًا `--claim-executor <child-id>`، ثم انتظر Markdown وstructured findings وسجّل `--complete-executor <child-id>`؛ يجب التحقق من design review قبل dispatch لمراجعة implementation plan
...
- بعد dispatch، استخدم `ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]` لكتابة native agent launch plan؛ يوجه controlling AI إلى آلية agent الأصلية في harness الحالي: Codex/GPT يستخدم `spawn_agent` مع harness wait lifecycle ولا يغلق إلا إذا كانت API متاحة، وClaude Code يستخدم Task، وGemini يستخدم `@generalist`، وOpenCode يستخدم `@mention`، وCursor يستخدم Agent/task chat، وCopilot يستخدم CLI/coding-agent task. استخدم `--json` عندما يحتاج adapter إلى machine-readable launch artifact على stdout. هذا الأمر لا يشغّل workers ولا أوامر shell بنفسه
...
- `IDE-CONTROLLER-AUTO-DISPATCH`: المستوى L1 للتقارير فقط. بعد اختيار المستخدم L2/L3، يملك IDE AI دورة tick -> توزيع كل `actions[]` عبر native subagents -> الانتظار -> تسجيل evidence -> tick فوري. إذا كانت `actions[]` فارغة مع وجود `pending` فهذه مراقبة فقط ولا يعاد التوزيع. لا تستخدم `loop watch` عند توفر IDE native subagent API
...
- يستخدم worker/reviewer logical model profile ويحل actual model من `.skillrc.workflow.model_profiles` حسب target، وإلا يعود بوضوح إلى harness default.
