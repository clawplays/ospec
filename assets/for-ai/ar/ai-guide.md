...
- الإغلاق تلقائي عند الجاهزية: عندما ينجح `ospec verify [changes/active/<change>]` ولا توجد required pending decision أو بوابة إضافة (plugin gate) حاجبة، شغّل `ospec finalize [changes/active/<change>]` بنفسك——لا تتوقف بعد نجاح `ospec verify` أو `ospec archive ... --check` (الـ `--check` معاينة فقط) ولا تنتظر طلب المستخدم. لا توقف الإغلاق إلا حين تحتاج بوابة فعلاً إلى إنسان: required decision غير مُجابة، أو بوابة إضافة حاجبة غير معتمدة (مثل Stitch أو Checkpoint)، أو blockers حقيقية يبلّغ عنها verify أو archive، أو طلب صريح من المستخدم للمعاينة أو الموافقة قبل الأرشفة. أنجز الإغلاق دائما عبر `ospec finalize` (أو `ospec archive`)؛ لا تنقل دليل change يدويا أبدا — إذ يؤرشف finalize في اللحظة نفسها brainstorm المرتبط بهذا change. وإذا أبلغ finalize عن عدم الجاهزية لأن مراجعة ما زالت تنتظر تحققا بشريا أو على الجهاز (`decision` ليس `APPROVED`)، فسجّل موافقة المستخدم المؤكدة في تلك المراجعة ثم أعد تشغيل `ospec finalize` بدلا من نقل الملفات
