...
- اعرض خيارات القرار عبر واجهة الأسئلة الأصلية للـ harness عند توفرها (في Claude Code: `AskUserQuestion`) قبل تسجيل الاختيار؛ وفي Claude Code يثبّت `ospec session hook --target claude --apply` هوكات تعيد تأكيد هذا العقد وتحجب بشكل صارم إرسال الـ subagents طالما هناك قرار required معلّق
