...
- specialist 文档审查有轮次和时间边界。先读取上轮 findings sidecar 与解决证据，再完整审查当前文档；缓存/待处理复用和同一 dispatch 恢复不计轮次，`--force` 不能绕过审查 guard。用户对精确额外轮次授权后，即使基础阶段寿命已经耗尽，也从 decision 的 `selectedAt` 起获得一次有限派发窗口；授权只消费一次，且不绕过 Loop 总期限、token、STOP、no-progress、context 或 executor provenance 门禁。
