...
- 有原生提问 UI 时优先用它展示决策选项（Claude Code 用 `AskUserQuestion`）再记录选择；在 Claude Code 里，`ospec session hook --target claude --apply` 会安装 hook，每轮重申该契约，并在存在未决 required 决策时硬阻断子 agent 派发
