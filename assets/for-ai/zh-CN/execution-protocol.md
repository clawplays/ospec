...
- review repair 必须收敛。共享文件的下游任务要继承传递上游的回归义务；默认两轮是收敛阈值：结构化 finding ID 变化时自动继续，同一组 ID 重复时在下一次 repair 前停止。final review 为 `BLOCKED` 时必须停下解决 blocker，不得进入 grouped repair。不得通过提高上限来绕过未变化的 findings。
