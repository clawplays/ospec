...
- `Announce-Before-Act`：绝不静默执行流程。用一句话宣告当前使用哪个 OSpec skill 及所处阶段；即将运行哪个 `ospec execute ...` 命令、会写出什么产物；派发多少个原生 subagent、走哪种机制（Claude Code 用 `Task`、Codex/GPT 用 `spawn_agent` 加当前 harness 的等待机制，仅在提供 close API 时关闭、Gemini 用 `@generalist`、OpenCode 用 `@mention`）；进度被门禁挡住时说明被什么挡住、如何解锁
...
- 推导或派发实现任务前，用 `ospec execute doc-review [changes/active/<change>] [--stage design|plan]` 生成文档 reviewer 交接包。specialist 模式要启动一个原生 reviewer 子 agent，立即记录 `--claim-executor <child-id>`，等待 Markdown 和结构化 findings 后记录 `--complete-executor <child-id>`；design review 验证通过后才能派发 implementation plan review
...
- goal 集成循环使用 controller 模式时，不要停在 goal/Loop 初始化，也不要让用户手工运行 Loop 命令。继续留在同一个 IDE 主 AI 会话；门禁就绪后运行 `ospec loop run [change] --once --json`，立即通过 IDE 原生 API 把每个 action item 作为一批 fresh native subagent 派发，等待结果、写回持久 evidence，并且无需用户再次提示就继续 tick；每个 worker 只读取 action 引用的 dispatch/review/verification packet，不要把整个 goal 粘贴给每个 worker
- `IDE-CONTROLLER-AUTO-DISPATCH`：L1 只报告；用户选择 L2/L3 后，由 IDE 主 AI 负责 tick -> 派发全部 `actions[]` -> 等待 -> 写 evidence -> 立即再 tick。`actions[]` 为空但存在 `pending` 时只观察，绝不能重派；IDE 有原生 subagent API 时不要使用 `loop watch`
