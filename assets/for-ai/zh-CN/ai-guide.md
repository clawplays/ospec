...
- dispatch 后用 `ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]` 写入启动计划；`runtimeAdapter` 只接受当前、target 匹配的模型原生 subagent capability，并给出 native primitive
...
- goal 集成循环使用 controller 模式时，不要停在初始化或让用户手工运行 Loop 命令。运行 `ospec loop run [change] --once --compact-json`，通过每个 action 的 `runtimeAdapter.selected.nativeSubagent` 执行，每个 child 用 heartbeat 认领一次，等待之间运行 `ospec loop poll`，只有 poll 报告 `tickNow=true` 时才完整 tick——全程无需用户提示；每个 worker 只读取引用的 packet
