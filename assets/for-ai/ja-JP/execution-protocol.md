...
- review repair は収束させる。共有ファイルを変更する downstream task は推移的 upstream の regression obligation を継承する。既定の 2 round は収束しきい値であり、構造化 finding ID が変化している間は自動続行し、同じ ID が繰り返された場合は次の repair の前に停止する。final review が `BLOCKED` の場合は blocker の解決まで停止し、grouped repair に進めない。変化しない findings を回避するために上限を引き上げてはならない。
