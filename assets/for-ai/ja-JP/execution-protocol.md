...
- specialist document review には round/time guard がある。前回の findings sidecar と resolution evidence を先に確認し、その後 current document 全体を review する。cache/pending reuse と同じ dispatch の recovery は round を消費せず、`--force` は guard を迂回しない。ユーザーが exact extra round を承認すると、base stage lifetime が尽きていても decision の `selectedAt` から 1 回の bounded dispatch window が開く。この grant は 1 回だけ消費され、Loop lifetime、token、STOP、no-progress、context、executor provenance の guard は迂回しない。
