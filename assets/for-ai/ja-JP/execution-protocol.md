...
- dispatch 後は `ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]` で native agent launch plan を書く。Codex/GPT は `spawn_agent` と harness wait lifecycle を使い close API がある場合のみ閉じ、Claude Code は Task、Gemini は `@generalist`、OpenCode は `@mention`、Cursor は Agent/task chat、Copilot は CLI/coding-agent task を使うよう controlling AI に指示する。adapter が stdout の machine-readable launch artifact を必要とする場合は `--json` を使う。この command 自体は worker 起動や shell command 実行を行わない。`ospec execute dispatch` が単一 task を作成する場合はこの launch plan を自動生成するため、別途 `ospec execute launch` を呼ばずに dispatch の next instruction から直接 worker を spawn できる。明示的な launch は複数 task の batch か plan 再生成のときだけ使う
