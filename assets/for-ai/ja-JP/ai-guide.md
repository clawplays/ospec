...
- `Announce-Before-Act`: ワークフローを黙って実行しない。どの OSpec skill を使い、どの段階かを 1 行で宣言し、これから実行する `ospec execute ...` コマンドと生成する成果物、ネイティブ subagent を何体・どの機構で派遣するか（Claude Code は `Task`、Codex/GPT は `spawn_agent` と harness wait lifecycle を使い close API がある場合のみ閉じる、Gemini は `@generalist`、OpenCode は `@mention`）、進行が止まったときに何のゲートがブロックしているかを伝える
...
- implementation tasks を導出または dispatch する前に、`ospec execute doc-review [changes/active/<change>] [--stage design|plan]` で document reviewer packet を作成する。specialist mode では native reviewer child を起動し、直ちに `--claim-executor <child-id>` を記録し、Markdown と structured findings を待ってから `--complete-executor <child-id>` を記録する。design review の検証後に implementation plan review を dispatch する
...
- dispatch 後は `ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|opencode|cursor|copilot|shell|generic] [--dry-run] [--json]` で native agent launch plan を書く。Codex/GPT は `spawn_agent` と harness wait lifecycle を使い close API がある場合のみ閉じ、Claude Code は Task、Gemini は `@generalist`、OpenCode は `@mention`、Cursor は Agent/task chat、Copilot は CLI/coding-agent task を使うよう controlling AI に指示する。adapter が stdout の machine-readable launch artifact を必要とする場合は `--json` を使う。この command 自体は worker 起動や shell command 実行を行わない
...
- `IDE-CONTROLLER-AUTO-DISPATCH`: L1 は report-only。ユーザーが L2/L3 を選択した goal では、IDE AI が tick -> 全 `actions[]` の native subagent dispatch -> wait -> evidence -> 即時 tick を担当する。`actions[]` が空で `pending` がある場合は観察のみで再 dispatch しない。IDE native subagent API が利用可能なら `loop watch` を使わない
...
- worker/reviewer は logical model profile を使い、actual model は `.skillrc.workflow.model_profiles` から target ごとに解決する。未設定時は harness default に明示的に fallback する。
