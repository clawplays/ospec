...
- change を agent、tool、worktree、shell、human operator の間で引き渡すときは、`ospec execute handoff [changes/active/<change>] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot|shell|generic]` で `handoff.json` と `handoff.md` を書く。このコマンドは project session brief snapshot、target tool mapping、safety rules のみを記録する
...
- dispatch 後は `ospec execute launch [changes/active/<change>] [--task task-id] [--target codex|gpt|claude|gemini|grok|opencode|cursor|copilot] [--dry-run] [--json]` で `launch-plan.json` と `launch-plan.md` を書く。target と一致し、有効期限内の current session capability だけが `runtimeAdapter.selected.nativeSubagent` を選択できる
...
- controller Loop が Goal を所有する場合、各 worker task 完了後の統合 review は `ospec loop tick [changes/active/<change>]` で発行し、実際の executor provenance に原子的に関連付ける。`ospec execute review ... --task <task-id>` を直接使うのは non-controller workflow のみとする。task-level review decision は `artifacts/reviews/tasks/<task-id>/review.md` に保存され、承認まで dependent task は dispatch されない
